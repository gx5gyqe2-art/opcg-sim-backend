...
def combine(wv, wp, nf_v, nf_p, dv=None, dp=None):
    ...
    # 1e-4 でも同じ読み方（2026-09-11・符号化 v14 で合成バッチの引きが変わり、絶対値の 1e-4 を
    # わずかに超える組み合わせが出た＝実測 1.20e-4 に対し**同じ numpy を加算順だけ変えた下限が
    # 4.06e-5**・`max_norm` は下限より小さい 3.4e-6）。**下限より良くなることはあり得ない**ので、
    # 絶対値だけで落とすとバッチの引き次第で緑にも赤にもなる。テストは
    # 「絶対値 1e-4 未満**か**下限の 5 倍以内」で見る（`max_norm < 1e-5` の判定は据え置き）。
    g["within_noise_floor_1e-4"] = bool(
        g["max_rel"]["1e-04"] <= 5.0 * g["noise_floor"]["max_rel"]["1e-04"])
    return g
