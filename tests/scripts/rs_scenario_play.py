    ...
    **両席に同じ設定**で渡す（省略＝serve 既定）。
    ...
    for _ in range(MAX_STEPS):
        ...
        move = game.decide(player_id, trace=tr, net=net_for[player_id], sims=sims,
                           ...
                           worlds=worlds, setup_box=setup_box, select_branch=select_branch)
    ...
    for name in names:
        ...
        for seed in range(args.seeds):
            result, steps_log, frames = _play_one(
                ...
                setup_box=args.setup_box, select_branch=args.select_branch)
