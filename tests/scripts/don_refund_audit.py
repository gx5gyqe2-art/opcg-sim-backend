...
体が場に残る）。2 は列挙あり（付与先にサトリを選ぶ手が合法手に出る）。3 は **c10 が
リーダー効果を起動しない**——ACTIVATE_MAIN は探索されている（128 sims で 11〜22 訪問）が
Q≈−0.03〜−0.09 で、リーダーに2枚付けて殴る DON_BOX（Q +0.10〜+0.15）に負ける。512 sims でも
変わらない＝価値ネットの盲点（ドンデッキから最大5枚を場へ出す効果を負に評価している）。
掘りの利益（体に4枚付与して早期に6000で殴る）は、この起動が選ばれない限り到達しない。
