...
DEFENCE = (
    ...
    ("d_stopped",     "止めた本数（枚数ではない・T131）",           False),
    ("d_guard_value", "守りの備えの額（止めて浮かせた分）",        False),
    # **T132**（ユーザ指示 2026-09-20「測ってみて」）——**式の形の違いだけ**（機構の推測ではない）。
    # `A` は本ごとに独立に `min` を取るが**守り手の手札は 1 ターンで 1 つ**＝**同じ札を使い回せることになる**。
    # **予告: 差は正・本数とともに増え・`A` の外れと正に相関する**（＝「攻撃の本数」の正体の候補）。
    ("d_budget_gap",  "財布を共有しない過大（独立 − 共有）",       False),
    # **組にした量**（T130）——`d_stopped` と `d_attacks` は逆向きで互いに相関するので、
    # **片割れだけ見ると打ち消し合う**。**予告: これが本体なら両記録で正・どちらの片割れより大きい。**
    # **T131 の訂正**: 引くのは**止めた本数**であって**切った枚数**ではない
    # （1 本止めるのに 2 枚使うことがある＝**本数から枚数を引くと単位が合わない**）。
    # **T130 の +0.256／+0.095 は枚数を引いた値**なので、§で測り直す。
    ("d_through",     "通った本数＝本数 − 止めた本数 − ブロック",   False),
)
