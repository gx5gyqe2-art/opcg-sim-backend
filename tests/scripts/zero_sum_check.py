...
（`docs/game_theory.md` §2-2・§25 の宿題 1・読み取り専用）。
...
`--blind`（両席の手札を符号化から落とす）を足すと私的情報の非対称そのものが縮むので、
**`abs_mean` が大きく下がれば、その分は情報差で説明できていた**ことになる。
...
def main(argv=None):
    ...
    ap.add_argument("--blind", action="store_true",
                    help="両席の手札を符号化から落とす（私的情報の非対称を消す）")
