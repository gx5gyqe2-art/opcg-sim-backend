...
VERIFIED_V2 = [
    # m1: CPU=シャンクス
    # m1@3 は**取り下げ済み**（2026-07-30 合意・反映は 2026-08-01）: ユーザの構造論
    # （序盤・手札減で出さない方が良い）を3系統クロス測定が支持し、v3 レフェリー裁定
    # （出すのが正・ウタ同格）と矛盾＝「測定では判別不能」が結論。判別不能な点を
    # 検証済み集合に置くと、両陣営が同じ手を指して全員 1.00 になり比較を薄めるだけ
    # なので採録しない（gen8 基準線は 4.7/14 → 3.7/13 に読み替え）。
    ...
    # m4: CPU=シャンクス
    # m4@2 の accept はユーザ裁定で拡大（2026-08-02）: プレイヤー観点は「何も出さない」が第一
    # （手札カウンター温存・先置きのメリット薄）。レフェリーの A&S&L 単独裁定はプラン列挙
    # （多ターン）由来で、単手因果 z≈0（v31 §2）＝測定では TURN_END と拮抗。当面の期待値は
    # 「TURN_END か A&S&L なら正・イワンコフ（ST30-004 で手札を回す）は band 外」とする。
    ("m4", 2,  {("TURN_END", None), ("PLAY", "OP13-007")}),
