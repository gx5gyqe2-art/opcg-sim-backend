...
2. **push 前に品質ゲート（`make test`）をローカルで実行し、全て green にする**。
   ...
   > **ゲートの頻度は下げる**（ユーザ決定 2026-09-18「ゲートは時間がかかるので頻度を下げてください」）——
   > **作業単位（T）ごとに回さない**。**複数の作業をまとめて 1 回**の `make test` ＋ 1 回の push にする。
   > 開発中の一次チェックは `make test-fast`（または触った範囲の `pytest tests/test_xxx.py`）で回し、
   > **`make test` は push 直前の最終ツリーに対して 1 回だけ**。**ゲート 1 回 = push 1 回**が基準。
   > **回数を減らすだけで、飛ばすわけではない**（CI が無いので push 前の green は必須のまま）。
   長く待つコマンド（アリーナ・生成）は**バックグラウンドで実行**し、完了を待つ間もチャット
   対応・他の作業を止めない（フォアグラウンドでブロッキング実行しない）。

   > **例外: 文書だけの変更はゲートを飛ばす**（ユーザ決定 2026-09-14）。
   > **根拠は実測**——**テストスイートは `.md` を 1 つも読んでいない**
   > （2026-09-14 に全 `tests/` を確認。`docs/` への言及は全部 docstring 内の参照）。
   > **文書だけの変更が `make test` を壊す経路は実在しない**ので、2 分を払う理由が無い。
   >
   > **「文書だけ」は目分量で決めず、機械的に確かめる**——`docs/` 配下と
   > **リポジトリ直下の `*.md`**（`CLAUDE.md`・`README.md`）**だけ**なら飛ばしてよい:
   >
   > ```bash
   > git diff --name-only origin/<branch>..HEAD | grep -vE '^(docs/|[^/]+\.md$)' | head
   > ```
   >
   > **1 行でも出たらゲートを回す。** `tests/` 配下の `.md`（fixture になりうる）・
   > `contract/`・`Makefile` は**文書ではない**。
   >
   > **ゲートを飛ばしたことは一報に書く**（「文書のみ・ゲート省略」）——
   > green と黙って同じ顔で報告しない。
...
> 副=固定ミラー）。**主条件の生成デッキは 2026-09-10 から `--decks synth_roles`**（除去の型を色ごとに差し込んだ
> 合成デッキ・実デッキに近い・ユーザ決定・`docs/rust_engine_plan.md` §20.8.8-3）。旧 `synth` は「旧分布への汎化」の
> 副次条件として必要なときだけ測る。r4 までの判定は `synth` で行った。1本の測定は昇格の証拠にならない——gen15 の「歴代初の昇格 0.5756」は
> 同条件で再現しなかった。判定は測定時のコミットに紐づき、エンジンを触れば失効しうる。
> **副条件の読み替え（ユーザ決定 2026-09-08・r3 採用時）**: 主条件（ランダム対面×生成デッキ＝汎化の
> 条件）が基準達成で、副条件（固定ミラー）が**退行なし**（勝率の 95% CI 下限 ≥0.45）なら採用してよい。
> 副条件が基準未達でも「同じ打ち回しの相手には差がつかない」だけで、退行ではないため。副条件の
> CI 下限が 0.45 を割れば従来どおり非昇格。
...
- **N系** = 効果構造符号化の学習CPU（カードID埋め込みなし）。2 世代ある:
  - **NRel（N系 v3・rN/aN）** = 22 枠の「効果構造＋今の状態＋ゾーン」トークン＋対の小ネット
    ...
    **現在の出荷既定は NRel r3**〔2026-09-08 採用・`nrel_r3.npz`・`opcg_sim/loop/engine.py::DEFAULT_NET`
    が正本（生成・アリーナ・serve が同じもの）・`docs/reports/r3_adoption_20260908.md`〕。**ロールバック先は
    a1**（同梱 `nrel_a1.npz`・2026-09-05 採用・`a1_adoption_20260905.md`）。構成は**関係 R なし・
    相手デッキ知識あり**（ユーザ決定 2026-09-05・訓練は `n_rel_train.py --ablate rel` が既定・系譜は
    r4, r5, … を続ける）。生成役も r3＝**era7**。
  - **c 系（cN）** = 旧 N系（`n_eff.py`・符号化 v12・最終 c10=2026-09-03 採用）。**Rust のネット
    読み込みは NRel だけ**なので c10 は serve のロールバック先にはならない（2026-09-07・切替後）。
    NRel の系譜（r3→a1→r1）がロールバック先。
...
## 理論フェーズの現在地と改善フロー（ユーザ指示 2026-09-17「忘れないように」）

- **毎セッションの最初に `docs/cpu_theory_gap.md` §0.6（現在地の地図）を読む**——目的・完成の判定（線形の橋と交点の橋の 2 つの形）・
  組み上がった層・残りの順番が 1 か所に在る。**T を 1 つ閉じるたびに §0.6 を書き換える**（数字は最新の報告から写す）。
- **理論は完全情報で組む**（ユーザ決定 2026-09-17・`cpu_theory_gap.md` §0.05）——**両席の手札・デッキが見える前提**で式を書き、
  まず**勝敗の説明**をつける。隠れ情報の推定は後の層。記録には両席の行が在るので、**1 行しか見ない器も相手の行と組にして両側を読む**。
- **理論の改善は §0.7 の 9 段のフロー**（問い → 式〔新定数ゼロ〕→ 検算の予告 → 器とテスト → 実・合成の両方で計測 → 不変量の確認 →
  報告 → 台帳 → ゲート・push・ユーザ判断）で回す。飛ばさない。残りの 1〜2 を閉じるたびに 2 つの橋を両方測り直す。

## ユーザに判断を求めるときの書き方（ユーザ指示 2026-09-24「今後は私に判断して欲しい時はこの形式で説明してください」）

判断を求める項目 1 つにつき、次の見出しをこの順で全部書く。**記号・変数名・定数名・切替名を使わない**
（「`s`」「μ」「`--seq attack_le`」ではなく「打った手が理論の最善よりどれだけ劣るか」「手札 1 枚ぶんの価値」
「付与を、それが準備する攻撃の価格で読む」のように言葉で言い換える）。

1. **何を決めるか**（1〜2 文）
2. **選択肢**（(a)(b)(c)… それぞれの帰結を数字つきで）
3. **なぜこの問いが出てきたか**（経緯——どこで見つかり、何を試して、どこで止まったか。時系列で）
4. **推薦とその理由**
5. **決めないとどうなるか**
6. **留保**（推薦を採っても残る問題）

複数の判断が並ぶときは、互いに独立かどうか（片方だけ先に決められるか）を最後に一言書く。
