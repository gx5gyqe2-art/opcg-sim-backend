...
- gamestate.py が 600行以下、最長関数 100行以下。
- `apply_action_to_engine` の文字列比較 0 箇所（enum キーのレジストリのみ）。
- 全品質ゲート green（ベースライン**無変更**・構造監査 0・slow journal pass・bench ±5%）。
- `docs/SPEC.md` のモジュール構成節が新レイアウトを反映。
