...
### 2.5.3 精度向上（計画）
現状の意思決定は `hard` でも 1-ply 貪欲が中心で、複数ターン先・相手の応手・効果連鎖を読まない。
対 CPU 体験を引き上げるため、以下を継続的な改善テーマとする（実装は段階的に進める）。

- **探索の深化**: 多 ply 先読み（自分→相手→自分）と、相手の最善応手を仮定した評価（ミニマックス／
  期待値）。`decide_guarded` の収束保証は維持する。
- **評価関数の高度化**: 重みの調整・学習、カウンター期待値／ブロッカー価値／効果連鎖（チェイン）や
  盤面テンポの織り込み、終盤（リーサル）認識の強化。
