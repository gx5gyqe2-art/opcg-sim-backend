...
**Phase 1.5: ターン粒度マクロアクション MCTS = 診断①②の本命解（実装済み・hard 超え達成）**:
  ...
  - **強さ（実測・対 hard 自己対戦・席交互・iters=160・horizon=3）= 18 局中 12 勝＝67%＝Elo 約 +120**＝**hard を
    明確に超えた**（micro の ~28% から逆転＝診断の妥当性を実証）。健全性ゲート（`tests/test_cpu_mcts.py`・macro 3本）=
    計画手は合法・manager 不変・決定論。本番 `decide` は不変（独立経路）。

**残（採用に向けて）**:
  1. **より大きな A/B で +Elo を確定**（18 局は有望だがサンプル小）＋**レイテンシ計測**（1 ターン一括計画＝
     `decide_cached`/ポンダリングと同じ replay で体感を隠せる前提の確認）。
  2. **`MCTS_MACRO_*`（horizon／iters／temp／C／PW）を自己対戦チューニング**（§2.5.3 の Elo A/B 枠組み）。
  3. **Phase 2: 決定化（ISMCTS）**＝相手手札を覗かず公開情報から N 通りサンプリング＝公平化（現状は hard 同様に
     相手手札を読む完全情報）。
  4. **本番配線**: 難易度 `hard`（または新 `expert`）への接続・PyPy ワーカー/計画キャッシュ統合。
  5. Phase 3（任意・大型）: 自己対戦で学習した方策/価値で sampling/value を置換（AlphaZero 風）。
