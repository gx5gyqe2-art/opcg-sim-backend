...
**強さ（実測・自己対戦・席交互・iters=160/H=3）— 正直な訂正あり**:
  ...
  - **⚠ 反復を増やすと悪化（重要な健全性の兆候）**: macro(no-plan) vs hard+plan を**同一 seed**で比較すると
    ...
    「攻撃が相手ブロック/カウンターを誘発するとターン境界が**戦闘の途中**になり評価が攻め側に楽観的」問題（未修正）。
    ＝現状の macro は「互角」だが**低反復で 1-ply サンプリング方策が効いているだけ**で、木探索自体は伸び代頭打ち。
    **葉評価の健全化（境界で戦闘を既定 PASS 解決してから採点する等）が、探索を増やして強くする前提**。
