...
### 2.5.7 MCTS（モンテカルロ木探索）= 人間並みを狙う探索刷新（`cpu_mcts.py`・Phase 1 MVP 着手）

**動機**: α-β＋ビーム（§2.5.2）は探索高速化が頭打ち（make/unmake・PyPy・PV 順序まで実装済みで逓減）。
さらなる強化は「速度」でなく **探索アルゴリズムそのもの**＝長期視点・隠れ情報に強い MCTS が王道（カード
ゲーム AI の定石）。現行 α-β `hard` は**温存**（A/B 基準＋フォールバック）し、MCTS は独立経路として
段階導入する。決定的エンジン＋自己対戦 Elo 基盤（`cpu_arena.py`／`elo_settle_ab.py`）が土台。

**Phase 1（実装済み・`cpu_mcts.decide_mcts`）**:
  - **完全情報 PUCT（AlphaZero 風）**: 相手手札も読む（`see_opp_hand=True`＝hard と同じ情報方針）。隠れ情報の
    決定化は Phase 2。
  - **方策ネットの代わりに 1-ply 評価のソフトマックスを prior**（`_score_move_1ply` を make/unmake で安く）＝
    少反復でも有望手へ集中。**価値ネットの代わりに静的 `evaluate` を葉の価値**に（[0,1] 圧縮・勝1/負0／盤面 tanh）。
  - **木**: ノード=決定点（手番側=`pending_actor_action()[0]`）。選択は 2 人零和 PUCT
    （acting==root は Q／他は 1-Q ＋ `MCTS_CPUCT·prior·√N_parent/(1+N_child)`・未訪問子は FPU=親推定）。
    1 反復ごとにルートを clone してパス再生。ルート最終手=最多訪問の子（robust child）。
  - **健全性ゲート**（`tests/test_cpu_mcts.py`・4本）: 合法手のみ返す／入力 manager 不変／同一 seed で決定論／
    単一手ショートカット。本番 `decide` は不変（独立経路）。

**micro 版（各ノード=1 手）の壁と診断（2026-06）**:
  - 対 hard 自己対戦（席交互）: 純 UCT(iters=80)=4/14≈29%／PUCT prior(iters=100)=5/18≈28%＝**hard 未満**。
  - **診断①（評価点バイアス）**: micro は**各ノード（ターン途中の局面）で評価**するが、α-β は「常に何かする」
    バイアスを消すため**ターン境界の静止点でのみ評価**する（§2.5.2）。micro は「行動したがまだ反撃されていない
    甘い局面」を過大評価し、その偏りが backup で木全体へ伝播する。
  - **診断②（整流コスト）**: 直す `_settle_eval`（葉をターン境界へ整流）は原理的に正しいが葉ごとに最大
    `_SETTLE_LIMIT` 手の効果解決を走らせ**実用反復数では計測不能なほど遅い**（8局×50反復が 10 分で未完走）。
    `MCTS_SETTLE_LEAVES` フラグ（既定 OFF）で保持。micro 版は参照/フォールバックとして温存。

**Phase 1.5: ターン粒度マクロアクション MCTS = 診断①②の本命解（実装済み・hard 超え達成）**:
  - **ノード=ターン境界の状態／エッジ=1 ターン丸ごとのプレイ**。これで (a) **葉が常にターン境界**＝`evaluate` が
    偏りなく・整流不要で安い（①②同時解決）、(b) **木の深さ=ターン数**＝激減し少 clone で複数ターン先を読める。
  - ターンは全列挙不能なので**確率的サンプリング**（1-ply 評価のソフトマックス方策 `_sample_turn_plan`）で候補を
    生成し、**progressive widening**（訪問増で子＝候補ターンを増やす）で木を広げる。選択は 2 人零和 UCB、葉は
    境界 `evaluate`（[0,1]）。ルート最善子=最多訪問のターンプラン＝**その手列を丸ごと返す**（`mcts_plan_turn`）。
    逐次 1 手 IF は `decide_mcts_macro`（`decide_cached` と同型＝queue に計画を持ち replay）。完全情報（Phase 1）。
  - **強さ（実測・対 hard 自己対戦・席交互・iters=160・horizon=3）**: 18 局 12 勝＝67%（Elo≈+120）→ **40 局 28 勝＝
    70%＝Elo 約 +147（大サンプルで確定）**＝**hard を明確に超えた**（micro の ~28% から逆転＝診断の妥当性を実証）。
  - **レイテンシ（実測・CPython）**: 1 ターン一括計画 `mcts_plan_turn` が **mean ~356ms／max ~405ms（iters160/H3）**。
    マクロは**ターンに 1 回だけ計画**して残りは queue を replay（`decide_cached` 同型）＝1 秒 envelope 内に余裕。
    ポンダリング（相手番中に前倒し）で体感は更に隠せる。PyPy なら約 2 倍速。
  - **Phase 2 決定化（公平モード・実装済み＝既定 OFF）**: `MCTS_DETERMINIZE`／`_determinize_opponent`＝相手の伏せ手札を
    「相手の山札＋現手札プールから同数を再サンプリング」した 1 通りに差し替えてから探索（root determinization）。自分の
    手札・手は実物＝**返すプランは実ゲームで合法**。これで「相手手札のカンニング」を除去する。完全 ISMCTS（反復ごと別
    世界）は uuid 整合が崩れるため探索 1 回 1 決定化＝単純で正しい第一歩。**強さ（公平 vs カンニングする hard・16 局）=
    9 勝＝56%＝Elo 約 +44**＝**カンニングをやめても依然 hard より強い**（公平化コストは +147→+44 で許容内）。
  - 健全性ゲート（`tests/test_cpu_mcts.py`・計 9 本＝micro4／macro3／決定化2）: 計画手は合法・manager 不変・決定論・
    自分手札不変・公平モードでもプランは実ゲーム合法。本番 `decide` は不変（独立経路）。全 1064pass・構造監査 0。

**残（本番採用に向けて）**:
  1. **`MCTS_MACRO_*`（horizon／iters／temp／C／PW）を自己対戦チューニング**（§2.5.3 の Elo A/B 枠組み）で +Elo を上積み。
  2. **本番配線**: 難易度 `hard`（または新 `expert`）への接続・PyPy ワーカー/計画キャッシュ/ポンダリング統合。公平モード
     （決定化）を本番の既定にするか（人間相手はカンニングしない方が望ましい）は要判断。
  3. **決定化の強化（将来）**: 複数世界平均（真の ISMCTS）・相手モデル（§2.5.4 テンプレ）に基づくサンプリングで精度向上。
  4. Phase 3（任意・大型）: 自己対戦で学習した方策/価値で sampling/value を置換（AlphaZero 風）。
