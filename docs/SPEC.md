...
- **探索（ターン境界評価・α-β＋ビーム）**: `decide` が難易度に応じて手を選ぶ。`_search` は葉の評価点を
  **`start_turn` から `horizon` ターン進んだ MAIN_ACTION（一定の静止点）に固定**する。自ターン内
  ...
  - **体感最適化（perceived latency・Phase 3・計画）**: 観点を per-call レイテンシ単体から「**ゲームを通して
    ...
    - **① 計画キャッシュ（contingency 部分木の抽出＆ルックアップ）★要石**: 探索は既に **min/max で相手の応手を
    ...
    - **⑥ ポンダリング（相手の番に ① を前倒し）★最重量・最後 — 設計（未実装・2026-06）**: 人間の手番中に ① の計画を
      ...
      - **本番有効化と実測（2026-06）**: 計測ハーネス（p1=人間プロキシ・p2=CPU で本番フローを asyncio 駆動）で hard 自己
        ...
        **実プレイの体感では ⑥-b が本命**: 人間は TURN_END した時点でもう打ち終わっており frontend の初回 `/cpu/step`
...
さらなる強化は「速度」でなく **探索アルゴリズムそのもの**＝長期視点・隠れ情報に強い MCTS が王道。現行
α-β `hard` は**温存**（A/B 基準＋本番既定）し、MCTS は独立経路として実験導入する。本番 `decide` は不変。
...
**強さ（実測・自己対戦・席交互・iters=160/H=3）— 正直な訂正あり**:
  ...
  - **公平モード（決定化）**でも対カンニング hard ≈56%（ハンデ hard 比較）＝チートをやめても遜色なし。
...
**不採用（このセッションで試し削除）**:
  - **α-β PV シード**（hard の 1 ターン計画を macro 木に保証候補注入＝下限を hard に固定）: **時間が最低 2 倍**
    （plan_turn＋MCTS）かつ **macro が hard に寄生**（地力が上がらず天井が hard）＝筋が悪く破棄。
