...
両側で**戦闘を安定境界まで既定解決してから採点**することで是正する。
  - **攻め側（Fix A・`MCTS_RESOLVE_COMBAT`／`_auto_resolve_opp`）**: 自分のターン中に攻撃が誘発する**相手の**ブロック/
    ...
    楽観評価を排除。
  - **防御側（`_score_defense_move`／`_settle_combat`・2026-06）**: CPU が**防御側**でカウンター/ブロックを選ぶとき、
    サンプラの 1-ply 採点が「カウンター宣言直後・戦闘解決前」（リーダーが一時的に強く見える）でなされ、**届かない部分
    カウンター**（例: 8000 攻撃にリーダー 5000＋1000＝6000 で“結局通る”）を稀に切る凡ミスがあった（実機プレイテストで
    観測＝Mr.3 を +1000 で空切り）。防御の戦闘応答手は**適用後に `_settle_combat` で戦闘を解決した実結果**（ライフ増減・
    カード消費・トリガー）で採点する＝無駄カウンターを正しく低評価しサンプリングから締め出す。**リーサルを 1 枚で弾ける
    カウンターは温存**（残ライフ1の検証で 30/30 でカウンター）。回帰: `tests/test_cpu_mcts.py`
    （`test_macro_defense_skips_unreachable_counter`／`test_macro_defense_blocks_lethal_with_reaching_counter`）。
    実測: 当該局面でカウンター空切り 3/40→**0/40**（PASS 40/40）。防御採点はクローン+解決のぶん重いが ~635ms/決定（160 反復・
    決定化）で本番デッドライン内。
