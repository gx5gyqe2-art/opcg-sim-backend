...
| `opcg_sim/src/core/gamestate.py` | ルールエンジン本体（ターン/戦闘/召喚酔い/5体上限/効果解決/除去保護/誘発キュー、`clone`/`get_legal_actions`/`default_interaction_payload`）。`apply_action_to_engine` は `core/actions` への1行デリゲート |
...
| `opcg_sim/src/core/rules_constants.py` | ルール定数（`SELF_RESTRICTION_KEYS` 等）。`actions`/`gamestate` が共有（循環回避のための葉モジュール） |
