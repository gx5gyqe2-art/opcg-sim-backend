...
**強さの実測と当面の壁（2026-06・正直な現状）**:
  - 対 hard 自己対戦（席交互）: 純 UCT(iters=80)=4/14≈29%（Elo≈−159）／**PUCT prior(iters=100)=5/18≈28%（Elo≈−166）**
    ＝**現状 hard 未満**。PUCT で prior を入れても低反復では gap が縮まらなかった。
  - **診断①（評価点バイアス）**: MCTS は**各ノード（ターン途中の局面）で評価**するが、α-β は「常に何かする」
    バイアスを消すため**ターン境界の静止点でのみ評価**する（§2.5.2）。MCTS は「行動したがまだ反撃されていない
    甘い局面」を過大評価し、その偏りが backup で木全体へ伝播する。
  - **診断②（整流コスト）**: 上記を直す `_settle_eval`（葉をターン境界へ整流して評価＝α-β と評価点を揃える）は
    **原理的に正しいが葉ごとに最大 `_SETTLE_LIMIT` 手の本物の効果解決を走らせる**ため、MCTS の反復数×手数で
    計算が爆発し**実用反復数では計測不能なほど遅い**（実測: 8局×50反復が 10 分で未完走）。`MCTS_SETTLE_LEAVES`
    フラグ（既定 OFF）で保持＝効率化（make/unmake 降下／ターン粒度マクロアクション木）が入るまで OFF。
  - **結論**: 成熟した α-β を MCTS で超えるのは**実装規模の大きい課題**。核心は (a) 評価をターン境界整合に
    しつつ (b) Python の clone/apply コストの中で十分な反復を確保すること。素朴な実装はどちらかで詰む。

**hard を超えるためのロードマップ（Phase 1.5〜3・優先順）**:
  1. **整流の効率化＋反復速度**: clone/反復→make/unmake 降下、葉の整流を「ターン粒度マクロアクション木」
     （ノード=自分の 1 ターン丸ごと＝整流不要で境界一致＋木が激減）で回避＝診断①②を同時解決する本命。
  2. **`MCTS_CPUCT`/`MCTS_VALUE_SCALE`/`MCTS_PRIOR_TEMP` を自己対戦でチューニング**（§2.5.3 の Elo A/B 枠組み）。
  3. **Phase 2: 決定化（ISMCTS）**: 相手手札を覗かず公開情報から N 通りサンプリングして各世界を探索＝公平かつ頑健。
  4. **Phase 3（任意・大型）**: 自己対戦で学習した方策/価値で PUCT を導く（AlphaZero 風）。
  本番採用は**対 hard で明確に上回る Elo を自己対戦で確認**してから（速度・レイテンシも 1 秒 envelope 内で評価）。
