...
| パス | 役割 |
|---|---|
| `opcg_sim/api/app.py` | FastAPI アプリのエントリ（`create_app()` シェル）。CORS＋lifespan を組み立て `routers`（束ねルータ）を include するのみ。分離済みモジュールの主要名を後方互換で再公開（ただしデッキ読込は `services.decks` を patch する。②C で薄いシェル化） |
| `opcg_sim/api/routers/` | REST/WS エンドポイント定義（**ドメイン別 APIRouter**）。`game`＝対局作成/アクション/状態/バトル／`cpu`＝`cpu/step`・リプレイ／`cards`＝カード/アセット/`health`／`decks`＝デッキCRUD／`sandbox`＝フリーモード／`rule`＝オンライン対戦。`__init__.py` が `ALL_ROUTERS` を束ねて単一 `router` を公開。ロジックは presenters/ws/services へ委譲（②C） |
