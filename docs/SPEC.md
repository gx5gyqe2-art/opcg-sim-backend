...
- **評価関数** `evaluate(manager, me)→float`: **J値理論**（J値＝白＝デッキ残＋トラッシュ／黒＝手札・
  ライフ・場・ステージ＝リソース。自分の J値を下げ相手の J値を上げるゲーム）に整合する形で黒リソースを
  加重する。具体的には **ライフ（最重要・非線形＝薄いほど 1 枚の限界価値が高い／45[J] ラインの危険）**・
  **手札（枚数＋カウンター値＝防御に回せる資源）**・**場（存在価値・総パワー・ブロッカー＝最終防御・
  アクティブ＝将来の攻め圧）**・アクティブ DON 差を加重する。KO／カウンター誘発／ハンデス等の「相手 +1[J]」
  は相手側の枚数・パワー低下として差分に自然に反映される。
- **探索**: `easy/normal` は各合法手を `clone()` 上で適用 → `evaluate` で採点する **1-ply 先読み**（貪欲）。
  `hard` は **フルクローン多 ply 先読み**（α-β ＋ ビーム）で、自分の連続手を読み、ターン終了後は相手の
  最善応答（ブロック／カウンター／相手ターン）まで min ノードとして辿る。探索木内で `winner` に到達する
  手順を最高評価とすることで **リーサル（削り切り）を認識**する。`decide` が最良手を返し、`decide_guarded`
  が暴走防止ガード（ターン内の手総数キャップ・起動 / ドン付与の繰り返しキャップ）でターンの収束を保証する。
  per-turn 制限の付け忘れカードはこのガードで顕在化する。探索規模は `HARD_DEPTH`／`HARD_BEAM`／
  `HARD_NODE_BUDGET`（1 回の `decide` が生成するクローン総数の上限）で制限する。
- **難易度**: `easy`=ランダム合法手 / `normal`=貪欲 1-ply / `hard`=J値評価つきフルクローン多 ply 先読み
  （α-β ＋ ビーム・リーサル認識）。
...
- **公平性**: `easy/normal` は隠れ情報（相手手札の中身・裏向きライフ）を見ない前提でクローンをマスク
  できる設計とする（チート防止）。`hard` はユーザ選択により「最強」を優先し、多 ply 先読みの相手応答
  シミュレーションにクローン上の相手手札（隠れ情報）も用いる別方針とする（§6 の視点マスクは `hard` 探索
  には適用しない）。

### 2.5.3 精度向上（実装済み／継続テーマ）
`hard` は **フルクローン多 ply 先読み（α-β ＋ ビーム）＋ J値理論ベースの評価関数＋リーサル認識** を実装済み
（§2.5.2）。さらなる強化の継続テーマ:
...
| `opcg_sim/src/core/cpu_ai.py` | CPU(AI) 意思決定（`evaluate`/`decide`/`_search`/`decide_guarded`）。J値評価・normal=1-ply／hard=多 ply 先読み（α-β＋ビーム・リーサル認識）・難易度・暴走防止 |
