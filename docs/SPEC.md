...
- **置換 sub_effect のネスト中断**: 置換（REPLACE_EFFECT）は除去解決の最中に走る入れ子の中断。
  ...
  **意図的に非対応のまま残す範囲（accepted limitations）**:
  - **バトル KO 経路**: バトル KO 置換は OP10-034／EB02-030 の2枚のみで、いずれも**任意**（「〜しても
    よい／できる」）。任意の拒否時は本来の KO を進める必要があるが、提示方式は KO 判定を先に
    skip するため拒否と相性が悪い（拒否でもキャラが残る誤り）。対象が少なく価値が低いため、
    バトル KO 置換は自動解決（任意=採用）のまま残す。
