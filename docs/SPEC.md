...
- **検証**: 改善は §3.1 の CPU 自己対戦ハーネスで決定論・インバリアントを保ちつつ、難易度間の
  ...
  温存）に信号が出ないため、**パズル/シナリオ回帰集**（正解手種が既知の局面）・**凍結ベースライン Elo**
  （版 N 固定相手に N+1 勝率＝絶対強度）・**regret ログ**（崖エラーの安価な代理）を併設する（下記
  「2026-06 外部レビュー収束」）。
...
- **B-1(b) カウンター強要〔静的クレジットは不採用→探索の相手カウンターモデルへ〕**（`_search` min ノード・
  `_power_cap`／`_effective_power`・§2.5.2）: **静的クレジット（有効パワー上限を `対象防御＋緩衝` まで引き上げ）
  は不健全と判明**＝付与ドンは相手ターン（探索の葉）で `get_power(False)` がパワーを乗せないため、上限を
  上げても葉に伝播しない。カウンター強要の価値は**相手が実際にカウンターする**ときのみライフ温存差として
  現れる。現状 normal は min ノードで `_consumes_hand_card`（手札カウンター）を除外＝相手は決して
  カウンターしない。**健全形＝相手 min ノードに推定カウンター緩衝（profile `defense_factor` 由来・既定
  ~2000-3000）付きのカウンター応答モデルを入れる**（緩衝内は相手が守り切る＝盛っても無駄／緩衝超で貫通＝
  余剰ドンを攻撃に振るのが正の手）。探索の状態的変更のためユーザ確認の上で着手。**以下は旧・静的案の記録**:
  グローバル最硬の単一上限をやめ、リーダー5000・各キャラのパワーを跨ぐ離散
  ボーナスで「新規に命中/KO できるか」を価値化（OPCG はパワー線形でなく閾値で動く）。**加えて
  「カウンター強要（圧）」を静的に価値化**＝攻撃する体の有効パワー上限を「対象の防御 ＋ 推定カウンター
  緩衝（相手モデル `defense_factor` 由来／既定 ~2000–3000）」まで引き上げ、その範囲のパワーを価値化。
  これにより**余剰ドンを殴る体/リーダーに振るのが正の手**になる。重大度=高・中規模。
  > 実プレイ報告（2026-06）: 自デッキにカウンターイベントが無いのに CPU が**余剰アクティブドンを攻撃に
...
- **B-3 深掘り集合に重要手を強制投入＋拡幅**（`_scored_search`）: ブロッカー設置・`_selection_moves` の
  全除去候補・逆算リーサル手は 1-ply ランクに関係なく深掘り集合へ強制投入し、`HARD_ROOT_BEAM` を 4→6〜8。
  1-ply の浅さで守備 setup 手を落とす取りこぼしを是正。重大度=高（手リスト改修のみ）。

**バッチC（敗着リスク低減）**
- **C-1 逆算 reach のブロッカー/カウンター控除**（`_plan_progress`・§2.5.5）: reach 本数から相手の可視
  ブロッカー数を減算し、隠れ分は `defense_factor` 由来のカウンター緩衝（例 2000〜3000）を控除、割引後でも
  届く時のみ加点。重大度=中。
...
外部AIレビューとの往復で確定した方針。**実装はこれから**（バックログ）。WBS と同期。
- **検証基盤を全変更のゲートに（最優先）**: 評価関数を触る前に症状を決定論的に固定する。**パズル/シナリオ
  回帰集**（正解手種が既知＝致死を取る／受けて生存／**ドンをクロックに変換**／カウンター温存。B-1 のドン
  症状を1本ピン留め）＋**凍結ベースライン Elo**＋**regret ログ**。`tests/` 新設。
- **B-1 の的を再定義**: 主役を「命中閾値クレジット」から**アイドルドンの末端減価**へ寄せる（上記 B-1）。
- **クロックは実装済み最重量項＝独立クロック項は不採用**（裏取り③）。
- **公開情報ベリーフ更新（新規・高ROI・フェア）**: §2.5.4 の静的テンプレに、**見えたカード（トラッシュ・
  既プレイ・公開）分だけ想定カウンター/ブロッカー密度を減算**する軽量更新を加える。追加クローン不要・
  相手手札の中身は読まない（フェア）。`normal` 保守モデルの過大な防御想定を是正。
...
- **耐性・特殊キーワードの評価（主要キーワードは実装済み・一部は残課題）**: §2.5.6 の脅威項で
  **ブロッカー（W_BLOCKER）／ダブルアタック／効果耐性「KOされない」／速攻／バニッシュ**を資産として
  明示加点済み（プラン供給時）。**「効果では選ばれない」・アンブロッカブル等**の表現はまだ未評価。
  → 脅威シグナルの語彙を拡充する（テキスト/フラグ検出）。
...
- **脅威/キーワード資産項**（`_threat_value`・`_side_score(threat_aware=True)`）: 場のキャラに対し、
  **ダブルアタック**（`W_KW_DOUBLE`＝リーダー打点2倍）・**効果耐性「KOされない」**（`W_KW_RESIST`＝
  除去されにくい永続体）・**速攻**（`W_KW_RUSH`）・**バニッシュ**（`W_KW_BANISH`）を加点する。ブロッカーは
