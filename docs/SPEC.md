...
- **探索（単ターン探索・ターン境界評価）**: `decide` が難易度に応じて手を選ぶ。`_search` は **α-β ＋
  ビーム**で**自分のターン1回**を読み、葉の評価点を **相手の MAIN_ACTION（相手のターン開始＝自分の
  ターンが完全に解決した直後）に固定**する（相手のターンへは潜らない）。自ターン内の自分のメイン手は
  max、自分のアタックへの相手のブロック／カウンター応答は min として読む。**全候補が「自分のターン完了
  後の同じ静止点」で評価される**ため、手番パリティ／horizon による「何もしない（パス）が不当に低く評価
  され、やる事が無くても何かしてしまう」バイアスが消える（例: 5000未満でドン付与してリーダーに通らない
  アタック＝純損は、パスより低く出て畳まれる。意図的レスト生成のための非貫通アタックは後続効果で盤面が
  良くなるなら正当に残る）。探索木内で `winner` に到達する手順を **ply 割引付き**で最高評価とすることで
  **最短リーサル（削り切り）を認識**する。予算/ply 上限での打ち切りは `_settle_eval` で TURN_END／既定
  解決により**ターン境界へ整流してから評価**する（自ターン途中の甘い局面で評価せず＝horizon の抜け道を
  塞ぐ）。`decide_guarded` が暴走防止ガード（ターン内の手総数キャップ・起動 / ドン付与の繰り返しキャップ）
  でターンの収束を保証する。ルート手は 1-ply で事前選別し上位 `HARD_ROOT_BEAM` 手＋`TURN_END`（パスの
  基準線）を深掘りする（手ごと均等予算 `HARD_PER_MOVE_BUDGET`・各ノード幅 `HARD_BEAM`・総 ply 上限
  `HARD_MAX_PLY`）。予算はレイテンシ予算（切れても settle で正しく境界評価される）で、1 手を実用域
  （平均 ~1 秒）に保つ。**守りの深読み（次の相手ターンのリーサルに備えた温存）は持たない**（静的な
  ブロッカー／カウンター／脅威評価に委ねる）＝継続テーマ B2-lite（§2.5.3）。
...
J値理論ベースの評価関数・単ターン先読み（α-β ＋ ビーム・ターン境界評価）・最短リーサル認識を実装済み
（§2.5.2）。さらなる強化の継続テーマ:

- **守りの深読み（B2-lite）**: 単ターン探索は相手のターンへ潜らない（守りは静的なブロッカー／カウンター／
  脅威評価に委ねる）。上位 K 手だけ相手のターンを1回グリーディにロールアウトして守り適性で再評価し、
  次の相手ターンのリーサルに備えた温存（ブロッカー／カウンター）を読む。レイテンシは K 回に限定。
- **探索の高速化・深化**: 置換表（メモ化）・move ordering の改良で予算内の読みを深める
  （`HARD_PER_MOVE_BUDGET` はレイテンシ予算・切れても `_settle_eval` で境界評価）。`decide_guarded` の
  収束保証は維持する。
...
- **探索地平線を越える効果価値**: 遅延誘発・長期の盤面アドバンテージなど `HARD_DEPTH` より先に
  価値が出る効果は見えにくい。→ 反復深化／評価関数への期待値織り込みで補う。
...
| `opcg_sim/src/core/cpu_ai.py` | CPU(AI) 意思決定（`evaluate`/`decide`/`_search`/`decide_guarded`）。J値評価・情報方針3分化（easy=正直1-ply・公開／normal=単ターン・リーダー推測／hard=単ターン・チート）・**単ターン探索・ターン境界評価**（葉=相手 MAIN_ACTION・`_settle_eval`＝horizon/手番パリティ是正）・α-β＋ビーム・最短リーサル認識・**効果の単一対象選択の探索分岐**（`_selection_moves`）・暴走防止・自デッキ勝ち筋プラン補正（§2.5.5）・**脅威評価**（`_threat_value`・§2.5.6） |
