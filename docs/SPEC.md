...
> 移行（出荷設計の正本＝**§2.5.8**）。本節の探索/評価の機構（clone・ビーム・settle 等）は現行も有効。
>
...
> `defense_factor`/`aggro_lean` の eval 補正・`profile` 引数）を撤去。**CPU 評価は L1（カード通貨）単一系統**
> ＝`evaluate_base` は `cpu_eval_v2.evaluate_v2(...)` を素返しする（`evaluate` はその上に既定素通しの学習価値
> ブレンド `_value_blend` を被せる薄いラッパー）。本節以下の「手書き J値評価」「`_side_score`」「ライフ concave
> ・薄域膝（`W_LIFE`/`W_LIFE_LOW`/`W_LIFE_HIGH`）」「相手モデル profile 補正」を述べた設計テキストは**廃止前の
> 設計記録（履歴）**。L1 で存続するのは `_power_cap`/`_effective_power`/`W_POWER_OVERCAP`/`DECK_DANGER`
> （いずれも `cpu_eval_v2` が import して使用）と `_value_blend`・PIMC・α-β/ビーム探索・手番計画/キャッシュ・
> L1 SPSA チューナ（`tests/eval_v2_spsa.py`）。
...
- **難易度＝1 系統**（`easy`/`normal` は **2026-06 に廃止**。`expert`(MCTS) も撤去。下表へ集約）:
  | UI | キー | 方策 | 相手情報 |
  |---|---|---|---|
  | つよい | `hard` | 単ターン先読み α-β＋ビーム＋**PIMC（決定化・§2.5.8）**＋学習価値の葉ブレンド。葉評価は **L1（カード通貨）単一系統＝`cpu_eval_v2.evaluate_v2`**（手書き J値評価は **2026-06-27 に撤去**／自デッキ勝ち筋プラン§2.5.5 は同日全廃＝plan 非供給のフラット評価） | **公平**（相手の伏せ手札を覗かず K 通り決定化して平均＝§2.5.8。`cheat` は診断用に明示指定のみ） |
...
> 同日全廃した。その結果 **`cpu_opponent_model.py` は孤児化**（本番コードからの import はゼロ）し、純関数
> `build_profile`（`OpponentProfile` を返す）だけが `tests/test_cpu_opponent_model.py` の単体テストで存続する。
> 以下は廃止前の設計記録（履歴）。
...
| `opcg_sim/src/core/cpu_ai.py` | CPU(AI) 意思決定（`evaluate`/`evaluate_base`/`decide`/`_search`/`decide_guarded`）。**葉評価は L1 単一系統＝`cpu_eval_v2.evaluate_v2` に集約**（`evaluate_base` が素返し・`evaluate` は既定素通しの `_value_blend` を被せる薄ラッパー。手書き J値評価／評価v2フラグ／effect-value 実験／相手モデル profile 補正は **2026-06-27 撤去**）・**情報方針 fair 既定＋PIMC 決定化＋学習価値葉ブレンド（§2.5.8）**（`cheat` は診断用に明示指定のみ）・**ターン境界評価探索**（`_settle_eval`＝horizon/手番パリティ是正）・α-β＋ビーム・最短リーサル認識・**効果の単一対象選択の探索分岐**（`_selection_moves`）・L1 で存続する評価補助（`_power_cap`/`_effective_power`/`W_POWER_OVERCAP`/`DECK_DANGER`＝`cpu_eval_v2` が import）・暴走防止（自デッキ勝ち筋プラン補正§2.5.5／脅威評価§2.5.6 は **2026-06-27 に全廃**＝plan 非供給のフラット評価） |
| `opcg_sim/src/core/cpu_eval_v2.py` | **CPU の現用葉評価（L1＝カード通貨）`evaluate_v2`**。トレース時は内訳を `out["v2"]` に出力。`cpu_ai` から `_power_cap`/`_effective_power`/`W_POWER_OVERCAP`/`DECK_DANGER` を import して使う。チューナ＝`tests/eval_v2_spsa.py` |
| `opcg_sim/src/core/cpu_opponent_model.py` | **【孤児化 2026-06-27】** リーダー推測の相手モデル（`build_profile`→`OpponentProfile`・§2.5.4）。profile ベースの eval 補正撤去により**本番コードからの import はゼロ**。`build_profile`（純関数）のみ `tests/test_cpu_opponent_model.py` の単体テストで存続 |
