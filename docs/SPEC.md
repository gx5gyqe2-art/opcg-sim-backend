...
- **難易度＝情報方針の 3 分化**（API キー `easy/normal/hard` は維持し、挙動を再定義）:
  ...
  | ふつう | `normal` | 多 ply 先読み（リーダー推測の相手モデル＋自デッキ勝ち筋プラン） | **公開のみ**＋テンプレ由来の想定（§2.5.4。実手札は読まない） |
  | つよい | `hard` | 多 ply 先読み（フルクローン・最強＋自デッキ勝ち筋プラン） | **full**（クローン上の相手手札＝隠れ情報も読む） |
...
- **耐性・特殊キーワードの未評価**: `_side_score` が読むキーワードは**ブロッカー／速攻のみ**。
  KO/効果耐性・「効果では選ばれない」・除去保護・ダブルアタック・アンブロッカブル等を**資産として
  加点していない**（保護価値は地平線内に相手除去が弾かれた時しか出ない）。→ キーワード項として
  明示加点する（ブロッカーと同様）のが最も効果が大きい。
- **コスト低減の未評価**: `evaluate` はコストを読まず、低減は「打てるようになった手」が合法手に
  出る形でのみ反映。「低減という潜在資源」自体は無価値。→ 打てる脅威の期待値として軽く価値化。
- **効果ターゲット選択の非最適化**: `_drain_own_interactions` は効果の対象選択
  （`RESOLVE_SELECTION`）を `default_interaction_payload` のヒューリスティック（最良 or 先頭）で
  自動解決し、探索の分岐にはしない。「効果で**どれを**KO/除去するか」は最善を読み切っていない。
  → 少なくとも KO/除去系のターゲットを探索分岐へ昇格、または既定ヒューリスティックの評価関数化。
...
> 「何もしない」を一級化（§2.5.2）。上記4点は本リストとして WBS に課題登録済み。
...
| `opcg_sim/src/core/cpu_ai.py` | CPU(AI) 意思決定（`evaluate`/`decide`/`_search`/`decide_guarded`）。J値評価・情報方針3分化（easy=正直1-ply・公開／normal=多ply・リーダー推測／hard=多ply・チート）・α-β＋ビーム・最短リーサル認識・暴走防止・自デッキ勝ち筋プラン補正（§2.5.5） |
