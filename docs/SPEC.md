...
- **難易度＝1 系統**（`easy`/`normal` は **2026-06 に廃止**。`expert`(MCTS) も撤去。下表へ集約）:
  | UI | キー | 方策 | 相手情報 |
  |---|---|---|---|
  | つよい | `hard` | 単ターン先読み α-β＋ビーム＋**PIMC（決定化・§2.5.8）**＋学習価値の葉ブレンド（自デッキ勝ち筋プラン§2.5.5 は **2026-06-27 に全廃**＝現在は plan 非供給のフラット評価） | **公平**（相手の伏せ手札を覗かず K 通り決定化して平均＝§2.5.8。`cheat` は診断用に明示指定のみ） |
...
- **C-2 テレグラフ致死の減点【撤去 2026-06-27】**（`evaluate`／`_telegraph_lethal`・
  ...
  項の isolate）。重大度=中〜高。
- **C-3 自他ライフの別カーブ＋膝位置の対面依存【実装済み・存続】**（`_side_score`／`_own_life_knee`・§2.5.3）:
  ...
  > **【2026-06-27 注記】** 本項は **plan ではなく相手モデル `profile`（`aggro_lean`）依存**のため、自デッキ勝ち筋プラン
  > 全廃後も存続する（`_own_life_knee`/`W_LIFE_LOW`/`W_LIFE_HIGH` は削除されていない）。旧 §2.5.5 参照と単体テスト
  > `tests/test_cpu_self_plan.py`（削除済み）への参照のみ無効。重大度=中。
...
> `normal` だったため孤児化）。**汎用のデッキ分類器 `cpu_opponent_model.build_profile` のみ残置**。
> 当初は自デッキ勝ち筋プラン（旧§2.5.5）が自分のデッキ構成を集計するのに使っていたが、その plan 系は
> **2026-06-27 に全廃**（§2.5.5 の撤去バナー参照）。現在 `build_profile`/`OpponentProfile` は相手モデル側
> （`_own_life_knee` の対面別ライフ膝・`_estimate_counter_buffer` の推定カウンター緩衝）で存続する。
> 以下は廃止前の設計記録（履歴）。
...
| `opcg_sim/src/core/cpu_ai.py` | CPU(AI) 意思決定（`evaluate`/`decide`/`_search`/`decide_guarded`）。J値評価・**情報方針 fair 既定＋PIMC 決定化＋学習価値葉ブレンド（§2.5.8）**（`cheat` は診断用に明示指定のみ）・**ターン境界評価探索**（`_settle_eval`＝horizon/手番パリティ是正）・α-β＋ビーム・最短リーサル認識・**効果の単一対象選択の探索分岐**（`_selection_moves`）・暴走防止（自デッキ勝ち筋プラン補正§2.5.5／脅威評価§2.5.6 は **2026-06-27 に全廃**＝plan 非供給のフラット評価） |
