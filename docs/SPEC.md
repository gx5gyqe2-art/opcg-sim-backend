...
| `OPCG_LOG_SILENT` | （未設定） | `1` で stdout/file シンクを抑止（テスト/診断用。バッファ蓄積は維持） |
| `OPCG_LOG_SINK` | （未設定＝自動分岐） | ログのシンクをカンマ区切りで明示指定 `{stdout,file,gcs,slack}`（指定時は最優先）。未指定時は GCS クライアントの有無で local/prod を自動分岐（§5.1） |
| `OPCG_LOG_DIR` | `logs` | `file` シンクの出力先ディレクトリ（1 セッション = 1 JSONL） |
| `LOG_BUCKET_NAME` | `opcg-sim-log` | `gcs` シンクのバケット名 |
| `SLACK_BOT_TOKEN` ほか | （未設定） | `slack` シンクのトークン／チャンネル（無ければ slack シンクは無作動） |

### 5.1 ログ／可観測性（log_event とシンク・CPU 思考トレース）

ログは目的の異なる **2 系統** が独立して流れる。

**① 一般ログ（`log_event`・`logger_config.py`）** — 「生成」と「転送先（シンク）」を分離している。

```
log_event(level, action, msg, player, payload)
  ├─ メモリ BACKEND_LOG_BUFFER[sessionId]（常時・FE ログとマージ用）
  └─ 有効シンクへ配る（OPCG_LOG_SINK / 自動分岐）:
       stdout … 1 行 JSON。Cloud Run では Google Cloud Logging に自動収集される
       file   … {OPCG_LOG_DIR}/<sessionId>.jsonl へ追記（GCS 不要・手元で grep/diff）
       gcs    … バッチ(FE+BE マージ)を save_batch_logs でブロブ保存
       slack  … level 別チャンネルへ通知（game./api./resolver. 等の接頭辞は除外）
```

シンクの既定（`OPCG_LOG_SINK` 未指定）は環境で自動分岐する:

| 環境 | 判定 | 既定シンク |
|---|---|---|
| ローカル | GCS クライアント不可・非サイレント | `stdout` + `file`（GCS 往復なし） |
| 本番（Cloud Run） | GCS クライアント可 | `stdout` + `gcs` + `slack`（従来同値） |
| テスト/診断 | `OPCG_LOG_SILENT=1` | （stdout/file を外す・バッファのみ） |

ログ出力は構造化 JSON（`shared_constants.json` の `LOG_CONFIG.KEYS`＝timestamp/source/level/
sessionId/player/action/msg/payload）。本番は stdout が Cloud Logging に入るため、`jsonPayload.*`
（action/player/sessionId 等）で検索できる（GCS ブロブを開かずに調査可能）。

**② CPU 思考トレース（挙動改善用）** — `log_event` とは別系統で、GCS を通らない。
...
（トレース有無で進行が分岐しない）。詳細・実行例は [`docs/TEST_SPEC.md`](TEST_SPEC.md) §3.2／§3.3。

**全ログフロー（action 名前空間の一覧・FE↔BE バッチ集約・Slack/GCS/Cloud Logging ルーティング・
CPU 思考トレースの全経路）は [`docs/LOGGING.md`](LOGGING.md) を正本とする。**
