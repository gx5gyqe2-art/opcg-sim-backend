...
> **更新（2026-06）**: 出荷 `hard`（α-β）の**情報方針は既定フェア**（相手手札を読まない）＋**PIMC（決定化）**
> ＋**学習価値の葉ブレンド**へ移行した。以下に残る「`hard`＝相手手札も読む」「`see_opp_hand=True` 既定」等の
> 記述は**歴史的経緯**で、現行の出荷設計は **§2.5.8** が正本。
...
- **難易度＝2 系統**（`easy`/`normal` は **2026-06 に廃止**。下表へ集約）:
  | UI | キー | 方策 | 相手情報 |
  |---|---|---|---|
  | つよい | `hard` | 単ターン先読み α-β＋ビーム（フルクローン・自デッキ勝ち筋プラン§2.5.5） | **full**（クローン上の相手手札＝隠れ情報も読む＝最強・チート） |
...
> **本節が出荷 `hard`（α-β）CPU の現行設計の正本**。§2.5.2 の「`hard`＝相手手札も読む（チート）」記述は
> 本節で**置換**された（出荷既定はフェア）。測定駆動で確定。各 Phase の計測は `docs/reports/` 参照
