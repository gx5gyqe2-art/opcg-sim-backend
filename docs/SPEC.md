...
### 2.5.1 配線・逐次進行
- **生成**: `POST /api/game/create` に `vs_cpu:true` / `cpu_difficulty`（`learned`＝既定・**gen12学習型**
  （`gen12_*.npz`・2026-08-05採用。**本世代の核は重みでなく探索の規約**＝「バトルを1つの箱として扱い、
  探索と value は『バトルをするかどうか』と『バトルの結果どんな盤面・手札・ライフになるか』だけを
  判断する」（ユーザ整理 2026-08-05）。3機構を既定 ON にして初めて機能する: `SERVE_QUIESCE`（葉が
  戦闘中なら解決してから評価）／`SERVE_BATTLE_READOUT`（実対局の防御窓は出口 value で選ぶ）／
  `TREE_BOX_BATTLE`（木の戦闘窓ノードも出口最良の1手へ畳む）。これで木・実対局・教師コーパスの
  解決規約が初めて一致した。重みは gen11 と「防御窓CF順位学習の腕」の **α=0.5 線形補間**（腕は
  `defense_cf_gen` の解決後盤面コーパス 19シャード2,198行584群を `rank_finetune_anchored` で学習。
  純腕は自ターン判断 m1@3 を劣化させたため補間で回収）。判定＝コーチゲート **6.19 vs 4.00（8点・
  退行ゼロ・ガード4点満点維持）**＝ m1@15「一度払ったら2000で止め切る」0.00→1.00・m2@44 0.00→0.62・
  m5@7 0.00→0.56 × アリーナ800局 0.5212 CI[0.489,0.554]＝中立（Elo+14.8）。**昇格基準（wr≥0.55）
  では FAIL のままユーザ判断で採用**（`docs/reports/gen12_adoption_20260805.md`）。policy は gen11 と
  同一バイナリ。旧世代へ戻すときは3機構とも False に戻す（片方だけ ON は train/serve skew）。
  以下は前世代 gen11 の経緯: gen10 に**蒸留アンカー付き順位学習**を積み、gen10 と **α=0.3 線形補間**。
  符号化は **v8**＝v7 + **自場集約3**（総火力/高パワー数/ブロッカー数。相手場（v5）と同じ関数の
  純対称化＝自場が生カウントのみだった非対称の解消。しきい値つき弱ボディ特徴は汎用性のため設けない）。
  教師は `option_pair_gen` のカード単位ペア（160局・296群・1,328ペア）で、**評価方法の2修正**を含む＝
  (1) ロールアウトの **def_temp=0.7**（argmax 防御は「手札を回して即出しする枝」に偽の優位を与える＝
  温存カウンターが使われる世界が生成されない）、(2) **margin_blend ラベル**（勝敗 z ＋
  0.25·clip(平均残ライフ差/4)＝拮抗群を勝ち方の質でタイブレーク）。学習は `rank_finetune_anchored`
  ＝順位ヒンジのバッチごとに「dense 一般盤面 16,000点で gen10 の予測へ引き戻す蒸留バッチ」を交互に
  流す（**アンカー無しの順位ヒンジは順位を上げるほど防御較正〈m2@12/58 の「素通しが正」〉を先に
  壊す**という v32 の負の結果を3回再現したことへの機構対応）。policy は gen10 と同一重みの v8 恒等拡張。
  判定＝コーチゲート v3 **PASS（6.9 vs 6.5・歴代最高）**・bar 超えは狙った m1@3 の改善（0.50→1.00＝
  **非発動イワンコフ**〈条件不成立で手札 6→5・バニラ2000 が湧くだけ〉を出さずウタを出す）のみで
  **退行ゼロ** × アリーナ 800局 0.4925 CI[0.463,0.522]＝中立（Elo−5.2）。
  **昇格基準（wr≥0.55）では FAIL のままユーザ判断で採用**（`docs/reports/gen11_adoption_20260803.md`）。
  decide 中央値は gen10 と同等（802→828ms・1秒予算内）。旧 gen10 以前はリプレイ再現/A/B/ロールバック用に同梱維持）／
...
- **sim 専用の対話自動解決** `default_interaction_payload`: 先読み中に `active_interaction` /
  `pending_request` が立った場合の機械的な既定確定。**ゾーン意味論**（`choose_selection`・2026-07-30）＝
  コスト系（自手札/場の候補）は min 件を価値昇順（安い札から払う）、獲得系（自山札/トラッシュ、
  公開一時領域 TEMP は min=0 の「〜まで」のみ）は max 件を価値降順（良い札から取る）、対象系
  （相手側候補）は価値降順。判別不能（ドン=RETURN_DON の候補順細工を維持・ライフ・混在・強制 TEMP）は
  従来の「先頭から min 件」へ退避。価値＝統一「残す価値」`card_keep_value`（コスト・現在パワー・カウンター値・効果保有・カウンタートリガー・【トリガー】の合成・探索分岐順序 `_rank_select_candidates` と共通）。
  旧既定（常に先頭 min 件・min=0 は常に0件）は、イワンコフ ST30-004 が公開した 6000 2枚を自分で
  捨てる／ウタ OP09-002 が公開山札から1枚も加えない実害があり、探索・自己対戦・レフェリー裁定を
  汚染していた（`tests/test_default_interaction.py` が実測2点で固定）。
  **これはクローン上の先読み専用**であり、本番（実対局・自己対戦）の未解決中断は
...
カードDB(日本語) ─ loader._create_card_master/make_parser ─▶ EffectParserV2
  ...
  ─▶ gamestate.apply_action_to_engine ─▶ core/actions（レジストリ・ディスパッチ：プレイヤーレベル/対象ループ）
  ─▶ gamestate(continuous / 除去保護 / 誘発キュー) ─▶ 盤面更新
```
...
| パス | 役割 |
|---|---|
| `opcg_sim/api/app.py` | FastAPI アプリのエントリ（`create_app()` シェル）。CORS＋lifespan を組み立て `routers.py` を include するのみ。分離済みモジュールの主要名を後方互換で再公開（ただしデッキ読込は `services.decks` を patch する。②C で薄いシェル化） |
| `opcg_sim/api/routers.py` | REST/WS エンドポイント定義（単一 `APIRouter`）。`/api/game/*`（`cpu/step` 含む）・`/api/deck/*`・`/api/sandbox/*`・`/api/rule/*`・`/api/cards`・`/health` 等。ロジックは presenters/ws/services へ委譲（②C・ドメイン別分割は followup） |
...
実アプリ対局    : create に cpu_trace=true（opt-in）→ CPU_GAMES[gid] にメモリ蓄積
                  ...
                  → GET /api/game/{game_id}/replay/frames で上記＋盤面フレーム列を一括取得
                    （リプレイビューア用。フレーム＝各アクション適用後のスナップショット・
                     action_index で actions/decisions と整合。正本は LOGGING.md「盤面フレーム」）
```
...
- **全カード挙動ベースライン `full_card_baseline.json`** は現状挙動の凍結。挙動を変えたら差分をレビューして `make regen-baseline` で更新する（テスト手順は TEST_SPEC §品質ゲート）。
