...
**Phase 1 MVP（実装済み・`cpu_mcts.decide_mcts`）**:
  - **完全情報 UCT**: 相手手札も読む（`see_opp_hand=True`＝hard と同じ情報方針）。隠れ情報の決定化は Phase 2。
  - **rollout を静的 `evaluate` で代替**: 複雑なカードゲームのランダムプレイアウトは遅く分散が大きいため、
    質の高い評価関数を**価値関数**として葉で使う（AlphaZero から方策/価値ネットを抜いた素朴版）。価値は
    ルート手番視点で [0,1] に圧縮（勝=1/負=0／盤面は `tanh(eval/MCTS_VALUE_SCALE)`）。
  - **木**: ノード=決定点（手番側=`pending_actor_action()[0]`）。選択は 2 人零和 UCT（acting==root は Q／
    他は 1-Q ＋探索項 `MCTS_C`）。手生成は α-β と同一（`_selection_moves`＋枝刈り）。1 反復ごとにルートを
    clone してパス再生（undo 不要＝正しさ優先）。ルート最終手=最多訪問の子（robust child）。
  - **健全性ゲート**（`tests/test_cpu_mcts.py`）: 合法手のみ返す／入力 manager 不変（clone 経路）／同一 seed で
    決定論／単一手ショートカット。本番 `decide` は不変（独立経路）。
  - **強さ（初期計測・対 hard 自己対戦・席交互・iters=80）= 14 局中 4 勝＝約 29%＝Elo 約 −159**＝**現状 hard 未満**。
    MVP は反復数が少なく（clone/反復が律速）・UCB 定数/価値スケール未調整・eval は hard と同一（MCTS の利得は
    選択的深掘りのみ）なため想定内の出発点。

**hard を超えるためのロードマップ（Phase 1.5〜3）**:
  1. **反復数を増やす速度**: clone/反復を make/unmake パス（パスに沿って適用→undo）へ置換＝§2.5.2 ②の再利用。
  2. **eval を事前確率（prior）に**: 展開順を eval 上位から（progressive widening＋PUCT 風 prior）＝少反復でも有望手に集中。
  3. **`MCTS_C`／`MCTS_VALUE_SCALE` を自己対戦でチューニング**（§2.5.3 の Elo A/B・案1 の重み最適化と同じ枠組み）。
  4. **Phase 2: 決定化（ISMCTS）**: 相手手札を覗かず公開情報から N 通りサンプリングして各世界を探索＝公平かつ頑健。
  5. **Phase 3（任意・大型）**: 自己対戦で学習した方策/価値で PUCT を導く（AlphaZero 風）。
  本番採用は**対 hard で明確に上回る Elo を自己対戦で確認**してから（速度・レイテンシも 1 秒envelope 内で評価）。
