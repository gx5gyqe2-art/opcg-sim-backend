...
- **探索地平線を越える効果価値**（残課題）: 遅延誘発・長期の盤面アドバンテージなど `HARD_DEPTH` より先に
  価値が出る効果は見えにくい。→ 反復深化／評価関数への期待値織り込みで補う（独立トラック）。
...
> **時間割引（地平線外の盤面価値の割引）**。残課題（多対象選択・**探索地平線を越える効果価値**）は WBS に課題登録済み。
