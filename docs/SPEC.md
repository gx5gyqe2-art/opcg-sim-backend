  ...
  - **体感最適化（perceived latency・Phase 3・計画）**: 観点を per-call レイテンシ単体から「**ゲームを通して
    ...
    - **① 計画キャッシュ（contingency 部分木の抽出＆ルックアップ）★要石**: 探索は既に **min/max で相手の応手を
      ...
      **PoC 実装済み（`plan_turn`・2026-06）**: 相手の介入（ブロック/カウンター）が入るまで、または TURN_END までの
      ...
      重ね）/⑥（ポンダリング）で前倒し分を隠すこと**＝計画キャッシュはその土台。**残**: decide パスへの配線（キャッシュ
      保持＋区切りでの再計画）・PyPy ワーカーへのオフロード対応。
...
- **ステージの評価【実装済み・Phase1.5】**（`_side_score`／`W_STAGE_COUNT`=800）: これまで `_side_score` は
  ...
  違い攻撃/ブロックに出ない永続リソースのため、**存在価値（`W_STAGE_COUNT`・キャラの `W_FIELD_COUNT` より軽い）
  ＋毎ターン価値を生むエンジン能力（`_recurring_engine` 成立時に `W_RECUR_ENGINE`）**のみ加点する（連続バフ＝
  聖地マリージョア等のコスト/パワー補正はキャラ側に既に反映済み＝二重計上を避ける）。存在は時間割引
  （`field_count_factor`）、エンジンは残ゲーム長で期待値割引（キャラのエンジンと同扱い）。ステージは可変・非対称
  （プレイ/除去/張替で増減）＝手選択に効く（自ステージのプレイ・相手ステージ除去の価値を拾う）。**プラン供給時のみ
  作動**（`engine_aware`＝plan is not None）＝plan=None では一切作動せず現行挙動と完全同値。ステージ不在時は
  `p.stage is None` で no-op＝キャラのみデッキの自己対戦・全ベースラインは不変。検証3本（`tests/test_cpu_self_plan.py`）。
...
- **【アタック時】を継続価値エンジン化【実装済み・Phase1.5・④】**（`_RECUR_TRIGGERS` に `ON_ATTACK` 追加）: 場のキャラの
  **【アタック時】(ON_ATTACK)** 効果は攻撃のたび誘発＝攻撃体が毎ターン生む将来価値だが、従来 `_recurring_engine` の
  対象外で**地平線を越える将来分が無評価**だった（地平線内の発動は探索が結果盤面で見る）。既存エンジン項
  （ACTIVATE_MAIN/PASSIVE 等）と同趣旨で `ON_ATTACK` をエンジン扱いに加え、`W_RECUR_ENGINE` の小プレミアム（残ゲーム
  長で割引）で補う。`_prune_futile_attacks` の `_attacker_has_on_attack`（無駄攻撃から【アタック時】持ちを除外）とは
  独立。プラン供給時のみ作動（`engine_aware`）＝plan=None 完全同値。検証1本。
