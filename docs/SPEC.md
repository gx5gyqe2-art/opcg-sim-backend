...
> **更新（2026-06）**: 出荷 `hard`（α-β）は**情報方針フェア既定＋PIMC（決定化）＋学習価値の葉ブレンド**へ
> 移行（出荷設計の正本＝**§2.5.8**）。本節の探索/評価の機構（clone・J値評価・ビーム・settle 等）は現行も有効。
...
- **難易度＝2 系統**（`easy`/`normal` は **2026-06 に廃止**。下表へ集約）:
  | UI | キー | 方策 | 相手情報 |
  |---|---|---|---|
  | つよい | `hard` | 単ターン先読み α-β＋ビーム＋**PIMC（決定化・§2.5.8）**＋学習価値の葉ブレンド（自デッキ勝ち筋プラン§2.5.5） | **公平**（相手の伏せ手札を覗かず K 通り決定化して平均＝§2.5.8。`cheat` は診断用に明示指定のみ） |
  | エキスパート | `expert` | MCTS（ターン粒度マクロ木・§2.5.7・公平モード） | **公平**（相手の伏せ手札を覗かず決定化して読む） |

  > **廃止経緯（2026-06・ユーザ判断）**: `easy`（1-ply 貪欲）と `normal`（リーダー推測の相手モデル§2.5.4＋
  > 公開情報のみ先読み）を削除し、**α-β=`hard` と MCTS=`expert` の2系統に集約**（既定 `expert`）。`decide` 系は
  > hard 方針に一本化（公開 API の `profile` 引数も撤去）。`normal` 専用だった相手モデル（§2.5.4 のリーダー推測・
  > `/api/cpu_template` 画面）も併せて廃止（`cpu_opponent_model.build_profile` は自デッキ分類＝§2.5.5 で使うため残置）。
...
- **公平性（2026-06 更新）**: 出荷 `hard`/`expert` とも**フェア**＝相手手札の中身・裏向きライフを読まない
  （`hard`＝`info_policy=fair` 既定＋PIMC 決定化・§2.5.8／`expert`＝MCTS 決定化）。`cheat`（旧 hard＝相手手札を
...
- **ステージの評価【実装済み・Phase1.5】**（`_side_score`／`W_STAGE_COUNT`=800）: これまで `_side_score` は
...
- **リーダーの可変状態の評価【実装済み・Phase1.5・②】**（`_side_score`／`W_FIELD_POWER`）: 従来 `_side_score` は
  ...
  戦闘価値（相手の硬い体を KO/トレード）を拾う。クロック（ライフ削り）は `_plan_progress` の reach が別軸で評価済み
  ＝二重計上を避ける（こちらは戦闘 KO/トレード軸）。プラン供給時のみ作動（`engine_aware`）＝plan=None 完全同値。検証2本。
...
### 2.5.4 リーダー推測の相手モデル（`normal`・テンプレートデッキ）【廃止 2026-06】
...
> `normal` だったため孤児化）。**汎用のデッキ分類器 `cpu_opponent_model.build_profile` のみ残置**＝自デッキ
> 勝ち筋プラン（§2.5.5）が自分のデッキ構成を集計するのに使う。以下は廃止前の設計記録（履歴）。
...
### 2.5.7 MCTS（モンテカルロ木探索・ターン粒度マクロアクション木）= 探索刷新（`cpu_mcts.py`）

**動機**: α-β＋ビーム（§2.5.2）は探索高速化が頭打ち（make/unmake・PyPy・PV 順序まで実装済みで逓減）。
さらなる強化は「速度」でなく **探索アルゴリズムそのもの**で、長期視点・隠れ情報に強い MCTS を独立経路として
実験導入する。現行 α-β `hard` は**温存**（A/B 基準＋本番既定）。本番 `decide` は不変。

**設計＝ターン粒度マクロアクション木（`mcts_plan_turn` / `decide_mcts_macro`）**:
  - **ノード=ターン境界の状態／エッジ=1 ターン丸ごとのプレイ**。これで (a) **葉が常にターン境界**＝静的
    `evaluate` が偏りなく・整流不要で安い、(b) **木の深さ=ターン数**＝激減し少 clone で複数ターン先を読める。
  - ターンは全列挙不能なので**確率的サンプリング**（1-ply 評価のソフトマックス `_sample_turn_plan`）で候補生成、
    **progressive widening** で木を広げる。選択は 2 人零和 UCB、葉は境界 `evaluate`（[0,1] tanh 圧縮）。ルート
    最善子=最多訪問のターンプラン＝**その手列を丸ごと返す**。逐次 1 手 IF は `decide_mcts_macro`（`decide_cached`
    同型＝queue replay）。`plan`/`profile` は任意（葉評価の勝ち筋項）。
  - **設計経緯（なぜマクロか）**: 各ノードを 1 手にする素朴な MCTS（micro・PUCT）は「ターン途中の甘い局面」を
    過大評価し対 hard ~28% と弱かった（α-β が §2.5.2 で消した手番パリティバイアスの再来）。葉をターン境界に
    限ると評価が偏らず、これが効いた。micro 版・葉整流 `_settle_eval`（葉ごと重く実用不能）は**不採用・削除**。

**強さ（実測・自己対戦・席交互・iters=160/H=3）— 正直な訂正あり**:
  - **当初「+147」は誤り**だった: 対戦相手の hard に `plan`（自デッキ勝ち筋・§2.5.5）を**渡し忘れたハンデ版**との
    比較（macro vs hard(no-plan) = 40局28勝70%）。hard は plan 前提で強いので、本番相当の比較で測り直すと:
  - **本番相当（両者 plan or 同条件）= ほぼ互角**: macro+plan vs hard+plan = 14/30≈47%／macro(no-plan) vs hard+plan
    = 16/30≈53%（いずれも 50%±ノイズ）。**leaf-plan 配線は macro に有意効果なし**（macro は実際に複数ターン読むため
    plan の致死/テンポ近似を必要としない＝plan は macro に必須でない。合成テストデッキ要因も寄与）。
  - **公平モード（決定化）**でも対カンニング hard ≈56%（ハンデ hard 比較）。
  - **レイテンシ**: `mcts_plan_turn` mean ~356ms/max ~405ms（CPython・ターンに 1 回計画→replay）＝1 秒 envelope 内。
  - **⚠ 反復を増やすと悪化（重要な健全性の兆候）**: macro(no-plan) vs hard+plan を**同一 seed**で比較すると
    iters=160→16/30≈53% に対し **iters=400→11/30≈37%（Elo≈−95）**＝**探索を増やすほど弱くなる**。健全な MCTS は
    探索増で同等以上になるはずで、これは**葉の価値にバイアスがあり探索が誤った収束先に集中している**兆候。最有力は
    「攻撃が相手ブロック/カウンターを誘発するとターン境界が**戦闘の途中**になり評価が攻め側に楽観的」問題。

**葉/採点の戦闘解決（中盤戦闘の楽観排除・実装済み）**: 上記バイアスは「戦闘応答の途中盤面で評価する」ことが原因。
両側で**戦闘を安定境界まで既定解決してから採点**することで是正する。
  - **攻め側（Fix A・`MCTS_RESOLVE_COMBAT`／`_auto_resolve_opp`）**: 自分のターン中に攻撃が誘発する**相手の**ブロック/
    カウンター（＋連なる選択）を既定 PASS で畳み、葉が常にきれいなターン境界（相手 MAIN）になるようにする＝攻め側の
    楽観評価を排除。
  - **防御側（`_score_defense_move`／`_settle_combat`・2026-06）**: CPU が**防御側**で応答を選ぶとき、サンプラの 1-ply
    採点が「宣言直後・戦闘解決前」（リーダーが一時的に強く見える）でなされ、**届かない部分防御**（例: 8000 攻撃に
    5000＋1000＝6000 で“結局通る”）を稀に行う凡ミスがあった（実機プレイテストで観測＝Mr.3 を +1000 で空切り）。防御手は
    **適用後に `_settle_combat` で戦闘を解決した実結果**（ライフ増減・カード消費・トリガー）で採点する＝無駄防御を正しく低評価し
    サンプリングから締め出す。**リーサルを弾ける防御は温存**（残ライフ1の検証で 30/30 でカウンター）。
    - **対象範囲は pending 種別ではなく「`active_battle` あり＆相手の手番」で判定**する。これで SELECT_COUNTER/BLOCKER
      だけでなく、**カウンター段階の前に解決される防御の誘発能力**（例: OP11-041 ナミ【ドン‼×1】【相手のアタック時】手札1枚を
      捨てる：リーダー+2000 ＝ `CONFIRM_OPTIONAL` 経路）も同じく戦闘解決後に採点される＝**凡ミスの別入口（リーダー能力で
      届かない+2000を空打ち→手札を無駄に捨てる）も塞ぐ**。検証: 届かない+2000 は USE スコア 2500→600 に低下し 20/20 で不発動、
      弾ける場合は 20/20 で発動。
    - 回帰: `tests/test_cpu_mcts.py`（カウンター: `..skips_unreachable_counter`／`..blocks_lethal_with_reaching_counter`、
      リーダー能力: `..defense_ability_is_treated_as_combat_response`／`..skips_unreachable_leader_ability`／
      `..uses_leader_ability_when_it_repels`）。実測: カウンター空切り 3/40→**0/40**。防御採点はクローン+解決のぶん重いが
      ~635ms/決定（160 反復・決定化）で本番デッドライン内。
  - **残**: iters=400 の劣化が両側の戦闘解決で解消したかは**再計測が必要**（本修正は実機で観測された凡ミスの是正が主目的・
    探索スケーリングの健全性回復は別途検証）。

**Phase 2 公平化（決定化・ISMCTS-lite・実装済み＝既定 OFF）**: `MCTS_DETERMINIZE`／`_determinize_opponent`＝相手の
伏せ手札を「相手の山札＋現手札プールから同数再サンプリング」した 1 通りに差し替えてから探索（root determinization・
自分の手は実物＝プランは実ゲーム合法）＝カンニング除去。`MCTS_WORLDS`>1 で複数世界アンサンブル（真の ISMCTS 近似・
先頭手で集約）。健全性ゲート `tests/test_cpu_mcts.py`（計画手合法／manager 不変／決定論／公平モードでも実ゲーム合法）。

**不採用（このセッションで試し削除）**:
  - **α-β PV シード**（hard の 1 ターン計画を macro 木に保証候補注入＝下限を hard に固定）: **時間が最低 2 倍**
    （plan_turn＋MCTS）かつ **macro が hard に寄生**（地力が上がらず天井が hard）＝破棄。

**残（本番採用・更なる強化に向けて）**:
  1. **macro 単体の地力向上**（hard 非依存）: 反復数増（レイテンシ余裕分）・反復の高速化（clone/反復→make/unmake）・
     `MCTS_MACRO_*`（C/temp/PW/horizon/value scale）の自己対戦チューニング。
  2. **評価関数の自己対戦最適化（SPSA・hard/macro 共通の `evaluate` を底上げ）**＝両エンジンに効く・推論コスト 0 増。
     前提＝**実デッキ評価環境**（`build_deck` を decklist 指定に拡張・合成デッキの過学習回避）。
  3. **本番配線＝実装済み（新難易度 `expert`・2026-06）**: hard（α-β）は温存し、**新難易度 `expert` を MCTS に配線**。
     `app.py` に `_plan_segment` ディスパッチを追加（`expert`→`cpu_mcts.mcts_plan_turn`(公平モード・インプロセス)／
     他→従来の `decide_client.plan_segment`(α-β・PyPy ワーカー)）。これ 1 点で**計画キャッシュ・⑥-a/⑥-b ポンダリング・
     即時 replay が全部 MCTS に乗る**（全機構が `plan_segment`＝1 ターン計画に集約されているため）。`/api/game/create` が
     `cpu_difficulty="expert"` を受理、`/cpu/step` は expert を MCTS 経路へ。フロント（GameStart）に「エキスパート」選択追加。
     公平モード（相手手札を覗かない）を既定に。配線スモーク `tests/test_cpu_ai.py::test_cpu_expert_difficulty_uses_mcts`。
     **注**: ポンダリングは `OPCG_PONDER=1` のとき作動（人間の番に MCTS を前倒し計算＝自ターン開始の待ちを隠す）。OFF だと
     自ターン開始に計画 1 回ぶんの待ちが出る。MCTS は当面インプロセス（CPython）＝PyPy ワーカー化は将来。
  4. **レイテンシ上限保証（壁時計デッドライン・2026-06）＝実機で必須**: 実プレイ計測で大盤面の `/cpu/step` が
     **p95 17s／max 34s** まで膨らんだ（MCTS は盤面が大きいほど turn-sampling が重く、ポンダリングでは隠しきれない）。
     対策として `mcts_plan_turn(deadline_ms=…)` を**ターンサンプリングの内側（`_sample_turn_plan`）まで伝播**し、どんな
     盤面でも上限を保証する（締切超過で部分プランを返す＝overshoot は 1 手決定ぶん）。expert は `_plan_segment` で
     `OPCG_MCTS_DEADLINE_MS`（既定 2000）＋`OPCG_MCTS_HORIZON`（既定 2＝大盤面の発散抑制）。決定性は壊れるので
     tests/自己対戦は `deadline_ms=None`（固定反復で再現可能）。実測: 巨大盤面で deadline=1000→1001ms と厳密にキャップ。
     **計画は CPU ターン開始に 1 回＋人間の防御で前提が崩れた時に再計画**＝待ちが出るのはその瞬間のみ（他手は queue replay で即時）。
  5. **評価関数の学習化（GBDT 価値葉）＝option value の穴埋め（計画・2026-06）**: 静的葉MCTSの**構造的な穴＝状況札の
     温存価値（option value）**を一般理論で埋める。現行の手書き `evaluate`＋切り詰め葉は即時/タクティカル価値（カード流・
     ドン経済）は捉えるが、**「今は対象が無い除去を将来の生対象に残す価値」は horizon 外＋静的葉では表現できない**（葉が
     未来までロールアウトせず、手札を汎用価値で価格付けするため）。一般解＝**自己対戦で学習した価値関数を葉に置く**
     （状態→勝率に option value が内在化＝明示的先読み不要・「iters 増で弱くなる」静的葉バイアスの同時改善も見込む）。
     デプロイ制約（**Cloud Run=CPU/リクエスト課金/push-to-main 自動デプロイ**・探索=**PyPy・stdlib-only**・Cloudflare
     Pages=静的UI）に合わせ、**GBDT(or 線形)をオフライン学習→pure-Python/JSON で同梱→葉で α ブレンド**（既定OFF・モデル
     無時は現 eval フォールバック）。学習は lightgbm 等をローカル開発時のみ（出荷物に入れない）・重みは `build_card_cache`
     と同方式でリポジトリ同梱（PyPy/CPython 双方で stdlib 読込）。費用は実質 $0（GPU/常時起動なし）・インフラ新規準備不要
     （push-to-main の既存自動デプロイに相乗り）。MVP 手順＝特徴抽出関数化（現 eval 内部項＋「除去/カウンター/ブロッカー
     在庫×相手脅威×ライフ×ドン」）→ローカル自己対戦でデータ収集→GBDT 学習＋エクスポート→葉ブレンド配線→自己対戦 Elo で
     α/反復数/deadline 調整→安全導入。これは項 2（SPSA）の上位互換的な実現（線形→GBDT で非線形交互作用を学習）。
     - **▼MVP 土台＝実装済み（既定OFF・2026-06）**: `cpu_features.extract_features`（30特徴・stdlib-only/PyPy互換・読み取り
       のみ決定論・`see_opp_hand=False` で相手手札の中身を読まない＝公平。**option value 交互作用項**＝除去在庫×相手場体数／
       ブロッカー在庫×相手場体数／カウンター総量×相手場合計パワー）。推論＝`cpu_value_model`（標準化＋ロジスティック回帰の
       pure-Python 推論・`value_model.json` 同梱・スキーマ不一致/未同梱は `is_available()=False` で現行 eval にフォールバック）。
       葉ブレンドは `cpu_mcts._value_boundary` に `(1-α)·tanh(eval)+α·winprob`（**`OPCG_VALUE_BLEND` 既定 0＝完全OFF＝推論を
       走らせず現状同値**・hard/α-β は不変＝expert葉のみ）。学習/収集は dev 専用（`tests/collect_value_data.py`／`tests/train_value.py`
       ＝pure-Python ロジスティック回帰・外部ライブラリ不要・ローカル/Cloud Run Jobs で実行）。健全性ゲート
       `tests/test_cpu_value_model.py`（特徴の長さ/決定論/manager非破壊/視点非対称/相手手札非漏洩・モデル読込/推論域/不一致拒否・
       **既定OFFで純eval同値**）。**同梱モデルは starter（easy 自己対戦 9314 行・val_acc 0.67）＝blend は OFF のまま**。本番投入前に
       hard/normal・実デッキで再学習し自己対戦 Elo で α を上げて検証する（線形→GBDT 化も同経路で差し替え可）。
       **学習データの観測点＝ターン境界の両者視点特徴＋終局勝者ラベル**は `core/cpu_value_data.py`（`turn_boundary_samples`/
       `label_samples`）に一本化し、自己対戦（`collect_value_data`）と**実対局のライブ採取**（`api/app.py` の
       `_capture_value_samples`＝**`cpu_trace` 時のみ・本番無影響**・replay endpoint が勝者でラベル確定し `value_samples` 同梱）が
       同一規約を共有する。**人間ログ活用**＝采取JSON→`tests/human_log_ingest.py`→JSONL、学習済みモデルの実戦汎化は
       `tests/eval_value_on_set.py`（外部セットで acc/logloss/Brier/ECE・読み取り専用・`predict_winprob(features, model=)` 経由＝
       推論の単一情報源）で測る。**弱点採掘(b)**＝`tests/value_gap_mine.py`（`realization_gap`）が采取の `value_samples` から
       予測勝率トラジェクトリを再構成し **value-realization gap**（勝者の最低予測勝率＝comeback_depth／敗者の最高＝
       throwaway_peak）で局をランク＝モデル/CPU が読み違える弱点局面を抽出（(A) データ補完の最優先候補）。
       詳細＝`docs/reports/cpu_human_log_pipeline_20260624.md`。

**人間観点の評価（実デッキ×公平モード・`tests/deckgen.py`）**: 「対 hard 自己対戦の Elo」は人間相手の良さの代理として
弱い（合成デッキ・自己参照・カンニング前提）。人間相手で効くのは**公平性（決定化＝相手手札を覗かない）・凡ミスの
なさ・実デッキでの堅実さ・速さ**。`build_realistic_deck`（リーダー色・イベント含む・4 枚積み・カーブ）で実カード
（効果フル）デッキを生成し、**公平モード macro vs hard** を実デッキで観察した結果:
  - **堅牢性 = 6/6 破綻ゼロ**（例外・スタック・不変条件違反なし）＝効果フルの実デッキ＋特殊リーダーでも安定。
    （副産物: 新エネル OP15-058 の「ドン!!デッキ 6 枚」常在ルールを `invariants.DON_CONSERVATION` が想定せず誤検知して
    いたのを修正＝`_expected_don_total`。エンジンは元々正しい。実デッキ検証が掘り当てた診断バグ。）
  - **レイテンシ（計画ターンのみ・公平モード・実デッキ）= median 685ms / p90 2281ms / max 4155ms**。手単位では大半が
    計画キャッシュ replay で即時だが、**自ターン開始の計画 1 回が重く tail が 1 秒目標を超える**＝人間 UX の主課題。
    対策候補: 壁時計デッドライン（計画を ~800ms で打ち切り）・大盤面ターンの iters 抑制・ポンダリング（人間の番に前倒し）。
  - 残: ① turn-start 計画のレイテンシ tail を抑える ② 相手モデル（§2.5.4）ベースの決定化サンプリングで読みを人間的に
    ③ 最終評価は**人間プレイテスト**（楽しさ・人間らしさ・凡ミスの有無は自己対戦で測れない）。

### 2.5.8 強さ=Elo 優先・フェア制約の出荷設計（2026-06・α-β に PIMC＋学習価値）

> **本節が出荷 `hard`（α-β）CPU の現行設計の正本**（§2.5.2 等の旧「チート既定」記述は本節へ更新済み）。
> 測定駆動で確定。各 Phase の計測は `docs/reports/` 参照
> （`cpu_strength_roadmap_20260622.md`／`cpu_phase1_horizon_sweep_20260623.md`／
> `cpu_phase3a_fair_value_relearn_20260623.md`／`cpu_phase4_pimc_budget_split_20260623.md`）。
...
- **留保**: 強さは ±100 級 CI で directional（未有意・確定には桁違いの局数）。`expert`(MCTS) は元から決定化＋
  学習葉を持ち、本節は**その強み（決定化・学習価値）を α-β(hard) へ移植**したもの＝両エンジン収束中
  （将来 hard≥expert を測って一本化候補）。人間ログ活用・非線形価値・hard/expert 統合は将来項（WBS）。
...
| `opcg_sim/src/core/cpu_ai.py` | CPU(AI) 意思決定（`evaluate`/`decide`/`_search`/`decide_guarded`）。J値評価・**情報方針 fair 既定＋PIMC 決定化＋学習価値葉ブレンド（§2.5.8）**（`cheat` は診断用に明示指定のみ）・**ターン境界評価探索**（`_settle_eval`＝horizon/手番パリティ是正）・α-β＋ビーム・最短リーサル認識・**効果の単一対象選択の探索分岐**（`_selection_moves`）・暴走防止・自デッキ勝ち筋プラン補正（§2.5.5）・**脅威評価**（`_threat_value`・§2.5.6） |
