...
バッチA-1/A-2/A-3 は実装済み（上記バックログ参照）。検証基盤はパズル集が実装済み・Elo/regret は残。
- **検証基盤を全変更のゲートに（最優先・一部実装済み）**: 評価関数を触る前に症状を決定論的に固定する。
  **パズル/シナリオ回帰集**（`tests/test_cpu_puzzles.py`・致死を取る／フェア性／脅威評価／ドン特性化ピン）
  は実装済み。**凍結ベースライン Elo**＋**regret ログ**は残。
