...
- **B-1 余剰アクティブドンの末端減価＋カウンター強要クレジット（有効パワーの対象別化を内包）**
  （`_power_cap`／`_effective_power`／`_side_score`／`W_DON_ACTIVE`・§2.5.2）: **主役はアイドルドンの
  末端減価**（OPCG は防御にドンを付与できず、カウンター無しデッキでは温存に実価値が無い）。これに
  カウンター強要クレジットと有効パワーの対象別化を併走させる。具体的には、グローバル最硬の単一上限を
  やめ、リーダー5000・各キャラのパワーを跨ぐ離散
