...
| `scripts/lethal_regret.py::_gen_positions` | 席共通・`pending_actor_action()`・per-game rng | 意思決定点の局面クローン収穫 | break |

エンジンのループ接点（pending request 種別・action_api の適用規約など）が変わると最大 6 箇所の追従が必要。
新計器を 1 つ作るたびにループを再実装しており、これが拡張の実コスト。

> 実装時の実測補正（2026-07-04）:
> - `scripts/lethal_audit.py` は依存 `collect_value_data`（学習eval レバー撤去時に削除）を import しており
>   **既に import 切れ＝死蔵**。移行対象ではなく **削除**（G-1）へ回す。
> - `scripts/lethal_regret.py::_gen_positions` は `get_pending_request()` でなく `pending_actor_action()` を使い、
>   対局を完走させず**特定条件の局面クローンを収穫するサンプラ**（per-game 派生 rng・グローバル budget 上書き）。
>   `run_game`（完走ランナー）へ押し込むのは無理筋なので**現状維持**（ドライバ対象外）。
...
新計器の追加＝observer を 1 つ書いて `run_game` に渡すだけ。`invariants` オプションは
`"raise"`（selfplay/replay/play_game）/`"skip"`（regret/realize＝検査なし・break 終了）の 2 系統。
...
- `cpu_replay`: **決定トレース（decision 行・card_id 基準）がバイト一致**＋step 行の**uuid マスク後**バイト一致
  ＋サマリ（winner/steps/turns＋card_id 決定列）一致。
  ※ step 行 payload の `uuid`/`card_uuid` は CardInstance 生成時の `uuid4`（seed 非依存・実行毎に変わる）
    なので**生バイト一致は原理的に不可**。決定論の本体は card_id 基準の決定列＝そこを固定する（既存設計方針）。
- `cpu_arena.play_game`: seed 0..19 の `(winner, steps, turns)` 一致（CRN あり/なし各系列）。
- `cpu_selfplay.run_one_game`: seed 0..9 のサマリ一致（policy=random/ai 両方。**完走局の一致＝軌跡全体の一致**）。
- `regret_trace` / `realize_trace`: seed 3/5 系列で集計 dict 完全一致。
...
**削除（G-1）** — git 履歴で復元可:
- 検証①完につき使い捨て 8 本（参照は凍結レポートのみ）: `_dissect_gen2_alldecks.py` /
  `_dissect_gen2_blackbeard.py` / `_fairdet_probe.py` / `_hunt_op16119.py`（参照ゼロ） /
  `_mine_gen2_selections.py` / `_verify_gen2_choices.py` / `_reproduce_op16080_waste.py` /
  `_reproduce_op16119_lifeadd.py`
- 死蔵 1 本: `lethal_audit.py`（`collect_value_data` 撤去で import 切れ・実行不能）
...
| G-4 | `cpu_arena` 3 ループ（`play_game`/`regret_trace`/`realize_trace`）移行、depth/thinktime 統合、再エクスポート撤去 | §2.6（arena/regret/realize 一致）＋全テスト＋文書追従 |
...
- エンジンのループ接点変更時の追従が 6 箇所 → 1 箇所。
- `tests/scripts/` 9 本削除（使い捨て 8＋死蔵 1）＋2 本統合、`cpu_arena` ~200 行の重複解消。
