...
- **ステージ2枠（改訂2・実装時に発見したステージ盲目の解消）**: 現行エンコーダは `Player.stage` を一切符号化して
  いなかった（常駐効果どころか存在自体が不可視）。v3 で card_idx を 22→**24** に拡張し**末尾**にステージ（自/相手）を
  追加。ネット側は平均プール対象を**先頭22枠に固定**（POOL_SLOTS）＝プールの分母が変わらないので恒等温スタート維持。
  ステージは EffFeat 射影経路（`stage_eff` 2×P）でのみ効く。
