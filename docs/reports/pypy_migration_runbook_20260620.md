...
- 採用の障壁は**配信スタックの PyPy 互換**：`pydantic-core`（Rust/PyO3）・`grpcio`（C 拡張・google-cloud 依存）。
  PyPy で歴史的に難物。
- **方式B（プロセス分離）**：Web/永続化は CPython のまま、**探索エンジンだけ PyPy ワーカー**で走らせる。
  互換問題を回避し、ロールバックも 1 フラグで可能。
- **方式A（単一プロセス全 PyPy）**は依存が PyPy で入れば最も単純。Phase 0 で可否を判定する。
...
## Phase 1B — プロセス分離移行（方式Aが通らなくても成立）
...
**Phase 0（半日・方式判定）→ 方式A or B → Phase 3 全緑 → Phase 4 段階切替。**
方式B は配信スタック無改造・1 フラグでロールバック可・挙動不変ゲート完備。
深さ転用（horizon +1）は切替後に A/B で別途。
