...
5. **【2026-07-10 追加修正】ワーカー間のシード重複**: 旧シードは batch_id のみ依存＝同round・同batch_id の
   ワーカーが**完全に同一のゲーム列**を生成（w1/w2 が終始重複していた実害＝実質1ワーカー分＋同一データの二重学習）。
   crc32(WID) をシードに混合して分離。**既走ワーカーはコード更新＋再起動が必要**（batch_id は枝から自動復元）。
