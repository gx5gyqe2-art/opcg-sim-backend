...
install できるか）** を実行した結果の記録（点の報告）。**結論：方式A 不成立・方式B 確定**。
...
- **方式A（単一プロセス・全 PyPy）＝不成立**。配信スタックの2大依存 `pydantic-core`・`grpcio` が
  PyPy でクリーンに入らない（wheel 無し／ソースビルドが拒否・長大）。事前予想どおり。
- **方式B（プロセス分離）＝確定**。CPython 側に `fastapi`/`pydantic`/`grpcio` を**そのまま残し**、
  **stdlib-only のエンジンだけ PyPy ワーカー**で回す。配信スタックの互換問題を**完全回避**。
- **版の注記**：テストは PyPy 3.9。PyPy **3.11** なら `pydantic-core` は Rust ソースから建つ*可能性*があるが、
  「wheel 無し＝毎回ソースビルド・PyO3 の PyPy 対応は実験的・`grpcio` は別途長大ビルド」で脆く、
  **結論（方式B 推奨）は変わらない**。3.11 で方式A を狙うなら Rust toolchain 同梱イメージでの再スパイクが前提。

## 次アクション
ランブック **Phase 1B（プロセス分離移行）** へ。最初の検証ポイントは **`GameManager` の pickle round-trip 一致**
（CPython↔PyPy 間で state を pickle で受け渡せるか）＝方式B の IPC 成否を握る。
