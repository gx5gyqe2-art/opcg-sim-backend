...
- **`model` は省略すると親を継承する**（実測: 子4件とも `claude-opus-5`）。既定は `claude-opus-5` とし、
  機械的な作業を大量に流すときだけ軽いモデルを検討する（§6 も参照）。
...
- **`create_session` の `model` 明示指定が通るか。** パラメータは存在し、**省略時に親のモデルを継承すること
  は確認済み**（子4件とも `session_context.model` = `claude-opus-5`）。別モデルを明示したときに実際にその
  モデルで起動するかは**未確認**。確認方法＝`model: "claude-sonnet-5"` で1件起動し、`get_session` の
  `session_context.model` を見る。**確認できたら §3.1 に追記する（既定は `claude-opus-5`）。**
