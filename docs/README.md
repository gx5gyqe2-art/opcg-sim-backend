...
| 文書 | 内容 |
|---|---|
| [`SPEC.md`](SPEC.md) | **システム仕様書**。全体アーキテクチャ／コアゲームルール（ターン・戦闘・召喚酔い/速攻・場5体上限）／オンライン対戦（ルーム・WS）／**CPU 対戦・AI**（§2.5）／カード効果システム／ファイルマップ／**ログ・可観測性**（log_event とシンク分離・CPU 思考トレース, §5.1）／既知のモデル化制約（§6.1） |
| [`TEST_SPEC.md`](TEST_SPEC.md) | **テスト仕様書**。テスト戦略／スイート一覧／診断・監査ツール／**効果検証ハーネス**（CPU 対 CPU 自己対戦・インバリアント検出, §3.1）／**CPU 思考トレース＋決定論リプレイ**（§3.2）／**ログのシンク分離**（§3.3）／品質ゲート／デッキ単位の手動検証 |
| [`LOGGING.md`](LOGGING.md) | **ログ仕様（全フロー）**。一般ログ（`log_event`・action 名前空間・シンク分離・FE↔BE バッチ集約・Slack/GCS/Cloud Logging）と CPU 思考トレースの全経路を 1 か所に集約した正本 |
