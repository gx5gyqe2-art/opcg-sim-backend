...
> 実装中の設計計画（一部未完・実装完了後に SPEC/TEST_SPEC へ吸収）: [`replay_verification_plan.md`](replay_verification_plan.md)（実対局リプレイ検証 R0-R3 実装済＋残少）／[`cpu_perf_testing_plan.md`](cpu_perf_testing_plan.md)（CPU 性能テスト運用 A1-A3 実装済）／[`cpu_v5_plan.md`](cpu_v5_plan.md)（**v5 学習計画**＝v4残課題「ドン付与ニュアンス」の解消。マーク局面シード＋探索/配管の要調査 S6/S7・22マークゲート）／[`refactoring_harness_driver.md`](refactoring_harness_driver.md)（検証ハーネス共通ドライバ化 ⑥）。
