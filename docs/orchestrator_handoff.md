...
→ **色特性＝きれいな3対3の分裂**: 赤/紫/黒＝0.542（L1互角）／黄/緑/青＝0.21〜0.38（L1に劣る）。青が最弱。
- **訓練量では説明できない＝本物の色特性**: 赤は664局で紫(813)・黒(835)と同点、青(538)・黄(640)は同程度の訓練量でも大きく劣る。
- **留保①（同一デッキ交絡）**: 各色は自分の色デッキで測定＝差は「学習しやすさ」だけでなく **L1がその色を上手く指すか（相性）** も含む（water-oil）。
- **留保②（青・緑が Gen0/SL基準0.45を下回る）**: 単なる伸び不足でなく **その色デッキ上でSL出発点より弱くなった疑い**（v2失敗と同型）。
  切り分けには各色のgen0を同じ色デッキで測る（§7の軽い代替）。**未実施＝次の司令塔タスク候補**。
...
- **構造チェック（発散なし）**: 全色 value.npz は NaN/Inf=0・ノルムは gen0(L2=38)→80〜90 で約2倍に収束、局あたり重み移動は最も進んだ紫が最小
  ＝「発散」でなく「プラトーへの収束」。劣化ではなく伸びしろ枯渇の絵。
