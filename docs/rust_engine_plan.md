...
木が深さ 2 で読む領域で、準備箱の「深さ 1」と同じ線引き。揃える点: 攻撃箱の中のアタック時効果の
**最初の対象選択も準備箱と同じ枝規則**にする（対象選択を持つアタック時効果があるときだけ・それ以外は
今のまま）。-1000 を「モリアに当てる枝」と「ゾロに当てる枝」を分けないと使い道が価値で決まらない。
...
■ 実装（§20.7.2 の決定どおり）
  ...
  6. 攻撃箱（attack_box_candidates）の中のアタック時効果に対象選択があるときは、その最初の対象選択を
     準備箱と同じ規則で枝にする（候補 ≤8・「選ばない」含む・2 つ目以降は既定解決）。対象選択の無い
     アタック時効果は今のまま。setup_box=true のときだけ有効（既定は不変）。枝数は RESULT に書く。
