...
| **D 守りを相手の攻撃パワーで層別** | `deck_kinds` とリーダー・相手の攻撃の DON 数で guard を割り、「安い攻撃を受けて高い攻撃を守る」になっているかを見る | guard はライフ 2〜5 で平ら（0.49〜0.54）・ライフ 1 でだけ上がる。一方で価値の帯はライフ 2〜4＝**守り始めが遅い**。交換レートの管理が在るかは未確認 | 小（`race_state` に列を足すだけ・読み取り専用） |
| **M リーダーの総ライフと「最後の 1 枚」** | ライフ 3 枚のリーダーだけライフ 1 での守りの価値が正（+0.048・n 524）。4 枚 −0.020／5 枚 −0.032／6 枚 +0.017 | 端の符号がリーダー依存＝「最後の 1 枚を守るか」だけは個別かもしれない。n が小さく手がかり止まり | 小（実デッキの検証帯で層別） |
