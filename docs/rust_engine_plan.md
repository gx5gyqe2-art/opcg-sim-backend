...
| **H 時間軸を補助ヘッドに足す**（ユーザ提案 2026-09-12・**2026-09-13 実装済み・訓練を配った**） | **実装した中身**: `time_head`（7 出力・Huber 回帰・`time_` 鍵・Rust は無視）＋ sidecar `*.time.npz`（`train/time_labels.py`）＋ `--time-weight`（既定 0.0＝1 bit も変わらない）。列は**決着までの自席ターン数（/10）・2／3 つ先の自席ターン開始時の自分と相手のライフ・決着直前のライフ**。**+1 ターンは既存の `aux` が持っているので入れない**。**「N ターン後の勝率」は z が対局内で定数＝記録から取ると教師が z と同一になる**ので、観測できるライフの軌跡で代用した（探索の自己評価 `pol_v0` を将来の行から引く版は観測ではなくブートストラップで v14 の波にしか無い＝信号が出たら次の段）。「どちらが先に落ちるか」も**勝敗とほぼ同義**なので単独の列にはせず、両者のライフの軌跡で表す | V は盤面 1 つに勝率 1 個を返すだけで「いつ」を持たない。ライフレースは速さの勝負。探索は時系列を持つが sims 160・worlds 4 では 5 ターン先の決着に届かない。既存の補助教師は 1 ターン先を予測しており機構は在る | 補助ヘッドの列追加＋教師の後付け（既存記録から作れる）。小〜中。**G と 1 本で試せる** |
