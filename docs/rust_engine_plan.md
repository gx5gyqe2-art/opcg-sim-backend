...
| **F 方針ごとの勝率を対局で使う** | ターンの頭で V_方針 を引き、argmax の方針で根の事前分布を偏らせる。両方の圏と序盤に限る  **2026-09-13 追加の根拠（時間 × 方針の地図・`2026-09-13_time_plan_map.md`）**: 手当ての向きが**2 点に確定**した——(1) **決着まで自席 2 ターン以内では守る**（guard 0.577 対 take 0.424＝+0.153・n 10,276・3 波一致）のに CPU は 74% 受ける・(2) **同じ帯で盤面を触るのをやめる**（face 0.684 対 board 0.081・n 11,949・3 波一致）のに 10.6% 触る。独立な 3 つの測定（相互リーサルの引き／攻撃パワー別の守り／本地図）が**すべて同じ向き**＝設計に迷いが無い。**残りターン数はネット自身が誤差 0.73 ターンで読める**（候補 H）＝バイアスの条件に使える。 **2026-09-13 の「しない／できない」の分離（`2026-09-13_can_or_wont.md`）で範囲が狭まった**: 守りは**止めるだけのカウンターが在った行だけでも +0.141**（受けた行の 92.6% は守る手段を持ち・71.0% は足りていた）＝**選択の誤りで確定**。一方**中盤（t5+）の board は 45% が強制**（リーダーを殴る手が無い）＝**触ってはいけない**。よって F の対象は**決着 2 ターン以内の帯だけ**にする。 | ネットの推奨は殴る 0.80／守る 0.78 なのに、実際は殴る 0.62／守る 0.40（`plan_head_r8`）。**層別で向きが群に依らないと分かった＝1 本の規則で全リーダーに効く**（`plan_map_strat`）。補助ヘッドのままでは対局に効かない（r8 のアリーナ 主 0.5000／副 0.4896） | Rust の変更あり（方針ヘッドの forward＋根の P の補正）。中〜大 |
| **G（優先度 下げ・2026-09-13）レースの状態を組み立てた量として入力に足す**（旧「案 A」） | リーサル圏の余裕・被リーサルの余裕・手数の差・守りの単価などを scalars の末尾に足す（append-only） | 勝敗に直結した唯一の箇所が両方の圏＝「攻撃回数 − ブロッカー − 相手ライフ − 1」の組み立てを要する局面。生の枚数は入力にあるが組み立てた量は無い | 符号化 v15（append-only）＋ Rust 側の同期。中 **優先度を下げた（2026-09-13）**: 時間 × 方針の地図で**攻めの方針の割合はレースの余裕で既に動いている**（face 0.356→0.580・board 0.239→0.078）と判った＝**入力に足りないのは「レースの量」ではない**。欠けているのは打ち回し側（候補 F）。組み立てた量を入れるのは F の後に、それでも足りなければ。 |
| **H 時間軸を補助ヘッドに足す**（ユーザ提案 2026-09-12・**2026-09-13 実施・判定済み: 無害かつ学べる・アリーナは動かない**） | **実装した中身**: `time_head`（7 出力・Huber 回帰・`time_` 鍵・Rust は無視）＋ sidecar `*.time.npz`（`train/time_labels.py`）＋ `--time-weight`（既定 0.0＝1 bit も変わらない）。列は**決着までの自席ターン数（/10）・2／3 つ先の自席ターン開始時の自分と相手のライフ・決着直前のライフ**。**+1 ターンは既存の `aux` が持っているので入れない**。**「N ターン後の勝率」は z が対局内で定数＝記録から取ると教師が z と同一になる**ので、観測できるライフの軌跡で代用した（探索の自己評価 `pol_v0` を将来の行から引く版は観測ではなくブートストラップで v14 の波にしか無い＝信号が出たら次の段）。「どちらが先に落ちるか」も**勝敗とほぼ同義**なので単独の列にはせず、両者のライフの軌跡で表す | **判定（r10・`2026-09-13_time_head.md`）**: holdout 19 万行の Huber が**学習後 0.279 対 定数 0.613 対 未学習 1.698**＝**盤面から時間が読める**（決着までの自席ターン数は誤差約 1.0 ターン・2／3 先のライフは誤差 0.8 枚）。3 波の行列は r7 から許容内（波 32 は −0.0069 で改善）。アリーナは主 +1.8／副 −19.9 Elo＝**ヘッドを足すだけでは強さにならない**（Rust の探索はこのヘッドを読まない）。**収穫は事実の方**: V が「いつ」を返さないのは情報が無いからではなく出口が 1 個しかないから。**次は読み出し**（時間 × 方針の地図）。 | V は盤面 1 つに勝率 1 個を返すだけで「いつ」を持たない。ライフレースは速さの勝負。探索は時系列を持つが sims 160・worlds 4 では 5 ターン先の決着に届かない。既存の補助教師は 1 ターン先を予測しており機構は在る | 補助ヘッドの列追加＋教師の後付け（既存記録から作れる）。小〜中。**G と 1 本で試せる** |

#### 未着手だが根拠のある候補

（B は 2026-09-13 に実施・判定済み＝下の「実施済み」へ移した。現在この節は空）

#### 実施して閉じた候補

| 案 | 結果 | 教訓 |
|---|---|---|
| **B 高分解能アリーナ r7 vs r5**（2026-09-13 判定） | **6 シャード 2,095 ペア（4,190 局・±10.5 Elo）で wr 0.4936 [0.4769, 0.5102]・−4.5 Elo [−16.1, +7.1]・void 5（0.24%）・dup 0**＝**r7 は r5 と同じ強さ**。事前登録は 2,800 ペアだったが、**残り 705 ペアが wr 0.59 級でない限り CI は 0.50 を割らない**（実質不可能）ので 6 シャードで締めた。w02・w05 は成果物を push せず終了＝帯 1022000／1025000 は未消化で再走可 | **192 ペアの Elo は 2 世代続けて誤差だった**（r5 vs r3 の +25 相当も 2,798 ペアで 0.5050）＝**192 ペアで世代交代を判断してはいけない**（`docs/measurement.md` §4 の `N ≈ (680/Δ)²`）。r7 は π の窓を 4 波に広げた世代なので、**「π は現 era のみ」の規約見直しは根拠が消えた＝現状維持**。生成役は r5 のまま。`docs/reports/2026-09-13_hires_arena_r7.md` |
...
| **A V と π のステップ比（`--pi-steps-mult`）** | **閉じる**（2026-09-13・r9＝×2／r9h＝×0.5・教材も warm-start も r7 と同一） | π を 2 倍**回して**買えたのは top1 +0.0026（3 波平均・二項の SE 0.0035 と同程度＝測れていない）・アリーナも r7 に及ばない（+14.6／+3.6 対 r7 の +25.4／+16.3）。**律速は回数ではなく相異なるデータ量**——同じ 1.6M 点の 2 周が +0.003 なのに、データ 2 倍の r7 は +0.052／+0.029／+0.011。**非対称が 1 つ残る**: π を半分にすると V が −0.009〜−0.012 改善し方策の代償は −0.005＝**`--pi-steps-mult 0.5` は「V を戻す安いつまみ」**として候補 C／F 用に残す。比とエポック数は絡む（r9 は best ep1・r9h は ep1 で過学習）。`docs/reports/2026-09-13_pi_steps.md` |
