...
**K1／K2（素の手を残す ＋ 束ね・つまみ `setup_box_keep_bare`・コーディネータ実測・`scenario_out_g/`）**

| 条件 | 神の裁き→攻撃 | 決着 | ガンマナイフ | KO（打った）| レイテンシ |
|---|---|---|---|---|---|
| K1: 箱 ＋ 素の手 ＋ R2 | 0/8 | 0/8 | **7/8** | 2/8（5）| 0.44 s |
| K2: K1 ＋ worlds 4 | 2/8 | 2/8 | 6/8 | 0/8（2）| 1.0 s |
| 参照: worlds 4 ＋ R2（箱なし）| 5/8 | 4/8 | 4/8 | 0/8（0）| 0.39 s |

- 素の手を残すとガンマナイフは戻る（7/8＝素の PLAY が拾う）。箱の KO 判断は残る（K1 で 2/8）。
  しかし神の裁き→攻撃の順は K2 でも 2/8（参照 5/8）＝**箱が根に居るだけで訪問が割れる**（束ねても
  代表の Q は 1 本ぶん）。
- **判定: 準備箱は今回は採らない**（既定 off のまま・`setup_box`／`setup_box_keep_bare`／`select_branch`
  はつまみとして残す）。買っているもの（KO 判断）は方策の候補行に対象を載せる符号化が入って初めて
  π に写る＝そのときに再測する。

**ゲート（合流後・本線）**: cargo test 421 passed・clippy clean・`make test` 482 passed・`make audit-cross`
120 ペア void 0。

**次（学習側へ）**: (1) 対称な補助教師（次の相手ターン／自分のターンの失うライフ・攻撃回数・効果発動回数
＋ 相手トークン単位の「攻撃したか・通した打点・発動したか」）を V の別ヘッドで学習（重み小）。
(2) 教材に対照を作る: 生成を worlds 4 ＋ R2 に・除去が合法な決定の数 % を ε 探索・synth デッキへの役割
（除去／減少／ロック）差し込みを `deck_dig.inject_dig` と同じ流儀で制御（実測 2026-09-09: synth 286 本中
除去 0 枚が 27%・平均 4.5 枚・除去も減少も無いデッキ 20%）。`deck_removal` を記録し、評価帯・アリーナを
リーダー別・除去率別に層別して読む。(3) §20.6（π を Q で補正）。(4) 符号化: 候補行に対象・
`power_owner_turn`・相手の起動メイン残数。
