...
| 生成 波25・26（**dump v2**・**era6＝生成役 a1**） | 2251000〜2258000／2261000〜2268000 | 指示済み（2026-09-05・ユーザ決定「R なし・相手デッキ知識あり」で a1 を出荷既定と生成役に。生成役 `claude/n1-results:n1_results/nrel_a1.npz`〔=同梱 `nrel_a1.npz`・0e0d649〕・`n_record_gen --dump-v2`・**32 セッション並列＝各波 16 シャード×480 局**（§7.3 の細分化・出力ブランチ `claude/n{W}-w01〜16`）・コード a1 採用コミット以降。r2 の訓練: π＝波25〜26（era6）・z＝波23〜26・warm-start a1・`--ablate rel`） |
...
生成役 a1 は c10 より約 1.7 倍遅い（960 局 ≒ 7 時間）ので 480 局で約 3.5 時間。

**NRel の訓練構成（ユーザ決定 2026-09-05）**: `n_rel_train.py --ablate rel`（関係 R なし・相手デッキ知識あり）が既定。系譜は r2, r3, … を続ける。
