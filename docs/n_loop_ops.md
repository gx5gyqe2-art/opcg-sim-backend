...
| 訓練 r2（era6 初回・オーケストレータ自身が実施） | —（seed 帯不要） | 指示済み（2026-09-05・**1 サイクル指示書**〔§7.4〕の一部。warm-start a1・`--ablate rel`・π＝波25〜26・z＝波23〜26〔OOM なら波23 を落とす〕・epochs 1〔4 本連続で best ep0 だった実測に基づく短縮・ユーザ未確認なら 2 に戻す〕・lr 5e-4。出力 `claude/train-r2` の `n1_results/nrel_r2.npz`＋log＋RESULT.json） |
| 検証 r2 vs a1 分散（生成に使った 16 セッションで実施） | ランダム 361000〜368000／ミラー 371000〜378000 | 払い出し済み（2026-09-05・候補 `neff:n1_results/nrel_r2.npz`〔claude/train-r2〕・基準 a1=出荷既定〔`--baseline` 空〕・16 セッション＝条件×シャード・各 24 ペア・出力 `claude/arena-r2-{random|mirror}-wNN`・判断基準 2 条件とも wr≥0.55 かつ CI 下限>0.50） |
