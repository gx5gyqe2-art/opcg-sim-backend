...
| 検証 切り分け a1／a2／c12 vs r1（主条件のみ・各 8 シャード×24 ペア） | a1 331000〜338000／a2 341000〜348000／c12 351000〜358000 | 指示済み（2026-09-05。候補=各ネット `neff:n1_results/{nrel_a1|nrel_a2|neff_net_c12}.npz`・**基準=r1** `--baseline neff:n1_results/nrel_r1.npz`〔n1-results 0e0d649〕・主条件のみ `--leaders random --decks synth`・24 セッション＝ネット×シャード・各 4 CPU・出力ブランチ `claude/arena-abl-{a1|a2|c12}-wNN`・台帳 `n1_results/arena_abl_dist/{a1|a2|c12}_wNN.jsonl`。読み方: 候補（遮断側）の勝率が 0.5 を有意に下回れば遮断した要因が効いている〔CI 上限<0.50〕・0.5 と区別できなければ効いていない。c12 が r1 に有意に負ければ構造の寄与、c12 が c10 相当かは別途 c12 vs c10 は測らず訓練 val で読む） |
