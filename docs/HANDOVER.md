...
**進捗**:
- ~~1. 検出ツールの追加~~ **対応済み（ラウンド3）** — `tests/mistarget_diagnostics.py`。
  detector A（PLAY_CARD zone=FIELD）/ B（REVEALED_CARD_TRAIT lift）/ C（デッキトップ公開だが LOOK無し）
  / D（TEMP消費だが LOOK無し）を件数＋カード番号で列挙。`scan()` が dict を返す。
- ~~2. 既知パターンの洗い出し~~ **対応済み** — ベースライン **A=0 / B=0 / C=8 / D=56**。
- ~~4. 回帰防止~~ **対応済み（ラウンド3）** — `tests/test_mistarget_guard.py` が A/B=0 を固定、C/D 上限を確認。
- **3. 既存対応の横展開（継続中・次の主標的）** — detector **D(56)** が最大の鉱脈。優先クラスタ:
  - **「このカードを手札に加える」→ `MOVE_CARD zone=TEMP` 誤解決**（`このカード`=self が TEMP 化）。
    `自分の手札に加える`系の自己参照が TEMP になる根を断つ（OP07-103/P-071/ST09-002/009 等, 10枚前後）。
  - **「（好きな順番で）持ち主のデッキの下に置く」→ `remaining_deck_bottom`(TEMP REMAINING) 誤ルート**。
    対象キャラ（コストN以下）を無視して残りTEMPを動かす no-op。`deck_bottom_general` 側で拾う
    （`好きな順番`＋対象指定ありは remaining ではない, OP06-056/058 等）。
  - **ライフ look 系**（OP10-022/OP13-105 等）— 「ライフ look-and-place」（§7-1 の難所）と同系統。
  - C(8) は大半が「任意のコストを宣言し相手デッキ公開」（コスト宣言メカニクス, §7-1 で別途保留）。

> 着手の起点: 本ブランチ PHoSv のラウンド1-3（§本ブランチ）が具体例＋修正パターン＋検出ツールの雛形。
> サイクル: `mistarget_diagnostics.py` で標的選定 → golden 追加 → ルール/エンジン是正 →
> `test_mistarget_guard.py` の上限を下げて固定。
