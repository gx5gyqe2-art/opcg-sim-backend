...
**マージ履歴（新しい順）**
- `claude/handoff-documentation-review-tM2nD` — **レスト制限「…レストにできない」の難所対処（PREVENT_REST 新設・9枚中7枚をネイティブ化・OTHER 64→56）（本ブランチ・作業中）**
...
| **tM2nD** | **レスト制限「…レストにできない」（PREVENT_REST 新設・アタック/ブロック封じ）** | **98.3%** | **56** | **+1** | **205** |
...
**残（同クラスタの未対応・別の構造難所）**: 9枚中 **7枚が直接ネイティブ化**。残る2-3枚は本ルールではなく
**上流の構造分解の難所**でブロックされている:
- EB02-011/EB03-017: 「〈場合〉、X。**その後**、（無条件で）…レストにできない」の形で、`場合` 節に続く
  「その後」の第2文が構造分解で脱落（rest-restrict 句が原子化される前に消える）。`parser.py` の
  「場合、A。その後、B」分割の課題（本クラスタ固有でなく横断的）。
- OP14-069: 「以下から1つを選ぶ」のバレットリストが `Choice(options=[])` と**空 options** で生成される
  （KO 選択肢も rest-restrict 選択肢も脱落）。`_parse_logic_block` のバレット Choice 解析の難所。
- いずれも次の標的候補（`§7 C` の Choice/その後 構造分解として別途対応）。
...
| C | 専用メカニクス・難所（要設計） | C11 完了・C7 一部完了(ライフscry)・C7.5 レスト制限完了(tM2nD)・C7残/C8-C10 残 |
...
~~**C7.5【完了 tM2nD】レスト制限「…レストにできない」**（9枚）~~
  - ~~「相手のコストN以下のキャラM枚までは、次の相手の（ターン/エンドフェイズ）終了時まで、レストにできない」~~
  - 対処: `PREVENT_REST`（ActionType 新設）＋ `rest_restrict` ルール＋ `timed_flags["CANNOT_REST"]`。
    レスト不可＝アタック/ブロック不可として `declare_attack`/`has_blocker` で弾く。9枚中7枚をネイティブ化。
  - **残（上流の構造難所）**: EB02-011/EB03-017（「場合、X。その後、Y」第2文脱落）・OP14-069（空 Choice options）。
...
| レスト制限: PREVENT_REST 新設（「…レストにできない」＝アタック/ブロック封じ・9枚中7枚, OTHER −8） | tM2nD |
