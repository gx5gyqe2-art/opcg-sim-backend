...
| 指標 | 刷新開始時 | PR#4 時点 | 現在 |
|---|---|---|---|
| 原子句カバレッジ（ルール命中率） | 0% | 約57% | **約93.0%** |
| `ActionType.OTHER`（実行時に何もしない句） | 942 | 421 | **209** |
...
> 本セッション（BWP0A）で「手札捨て」split 断片修正：カバレッジ 92.5%→93.0%（+0.5pt）、
> OTHER 234→209（−25件）、テスト 115→118（+3件）。退行=0 を全ラウンドで維持。
...
1. **裾野の OTHER／フォールバックのルール化** — 頻度は低く多様（上位でも10件前後/表現）。
   ...
   **対応済み**（本セッション）。残るのは**構造的/UI/専用メカニクス課題が中心**で、
   ...
   - ~~**構造断片「自分の手札1枚を」（11件）**~~ **対応済み**（本ブランチ）。
     `_parse_to_node` の `捨て、` を `(?<=捨て)、`（lookbehind）に変更し、動詞 `捨て` を
     前クローズに残すことで断片化を解消。あわせて `discard` ルールの `捨てる` チェックを
     `捨て` に緩和し `捨ててもよい`（連用形）も対応。OTHER 234→209（−25件）、退行=0。
