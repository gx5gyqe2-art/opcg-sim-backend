...
- `claude/handoff-docs-review-PHoSv` — **デッキ公開→条件付き登場の構造修正・隠れ不具合検出ツール新設（本ブランチ・作業中）**
...
| **A** | **隠れミスターゲット横展開**（D detector=56 が最大の鉱脈） | **進行中** |
| B | デッキ公開→登場クラスタの残フォロー | 一部残 |
| C | 専用メカニクス・難所（要設計） | 未着手 |
| D | 裾野 OTHER 継続 burn down（OTHER=104） | 継続 |
| E | 置換効果（REPLACE_EFFECT）の残 | 一部残 |
| F | アーキテクチャ整理（catalog 縮退） | 未着手 |
| G | 回帰防止（C/D detector 上限の随時更新） | 継続 |
...
**A1【最有力次標的】「このカードを手札に加える」→ `MOVE_CARD zone=TEMP` 誤解決**（約10枚: OP07-103/P-071/ST09-002/009 等）
  - `このカード` を self 参照と認識せず TEMP ターゲットになっている。`自分の手札に加える` 系の自己参照 TEMP 化の根を断つ。
  - 対処案: `このカード` / `このキャラ` を含む MOVE_CARD 生成ルールに `zone=TEMP` 除外ガードを追加。

**A2 「（好きな順番で）持ち主のデッキの下に置く」→ `remaining_deck_bottom`(TEMP REMAINING) 誤ルート**（OP06-056/058 等）
  - 対象キャラ（コストN以下）を無視して残り TEMP を動かす no-op。
  - 対処案: 「好きな順番」＋対象指定あり は `remaining_` ではなく `deck_bottom_general` 側で拾う。
...
**B5 scry-1**（OP08-049）
  - 「（デッキの一番上を）公開し、そのカードをデッキの上か下に置く。そのカードが…の場合、【速攻】を得る」。登場を伴わない別型。「上か下」選択 UI 未実装。

**B6 OP12-058 の `登場させた場合…【速攻】`**
  - `PREV_ACTION`（直前アクション成否）での条件分岐生成が未対応。`PREV_ACTION` ConditionType 自体は実装済み（bIYkG）。
...
**C11 `MOVE_CARD` の `dest_position` 欠落**
  - 「ライフの上に加える」の上下区別が未実装（現状は常に末尾=下）。
  - 対処案: `dest_position: Literal["top","bottom"]` フィールドを IR に追加し、エンジンの挿入位置を分岐。

---

### D. 裾野 OTHER 継続

**D12 OTHERランキング継続 burn down**（現状 OTHER=104）
...
**D13 REST_DON コストの充足判定**
  - 【ドン!!×N】コストで `REST_DON` を使う句。現状 `_can_satisfy_node` で target=None のためドン枚数検証が走らず常に True（コスト不足でも発動できる）。
  - 対処案: `REST_DON` の cost 判定に現在の rest DON 枚数チェックを追加。
...
**G19 C/D detector 上限の随時更新**
  - `tests/test_mistarget_guard.py` の `assert len(...) <= N`（現在: C≤8 / D≤56）。
