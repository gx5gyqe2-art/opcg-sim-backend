...
| 指標 | 刷新開始時 | PR#4 時点 | main（BWP0Aマージ後） | PHoSv（main マージ済） | **LM00j（本ブランチ）** |
|---|---|---|---|---|---|
| 原子句カバレッジ（ルール命中率） | 0% | 約57% | 約95.4% | 95.9% | **96.7%** |
| `ActionType.OTHER`（実行時に何もしない句） | 942 | 421 | 108 | 104 | **102** |
| 未分類条件 `GENERIC`（誤発動の温床） | — | 251 | 94 | 1 | **1（変化なし）** |
| パーサルール数 | 0 | 15 | 49 | 51 | **55（+4）** |
| テスト総数 | 17 | 43 | 137（全緑） | 176（全緑） | **～185（全緑）** |
| 本番パーサ | レガシー | EffectParserV2 | EffectParserV2（既定） | EffectParserV2（既定） | **EffectParserV2（既定）** |
