...
| 指標 | 刷新開始時 | PR#4 時点 | 現在 |
|---|---|---|---|
| 原子句カバレッジ（ルール命中率） | 0% | 約57% | **約87.6%** |
| `ActionType.OTHER`（実行時に何もしない句） | 942 | 421 | **263** |
| 未分類条件 `GENERIC`（誤発動の温床） | — | 251 | **94** |
| パーサルール数 | 0 | 15 | **39** |
| テスト総数 | 17 | 43 | **111（全緑）** |
| 本番パーサ | レガシー | EffectParserV2 | **EffectParserV2（既定）** |

> 本セッション全体でカバレッジ 70.4%→87.6%（+17.2pt）、OTHER 333→263（−70件）、
> ルール 25→39（+14種）、テスト 81→111（+30件）。退行(新規OTHER)=0 を全ラウンドで維持。
...
1. **裾野の OTHER／フォールバックのルール化** — 頻度は低く多様（上位でも10件前後/表現）。
   ...
   rush_natural / reveal_hand（手札公開）は **対応済み**（本セッション）。
   次の候補（現在のランキング上位）。**上位は構造的/UI 課題で単純なルール追加では解けない**:
   - **サーチ「デッキの上からN枚を見て…カード1枚までを公開し、手札に加える。残りを…」（13件＋）** —
     最有力だが構造的。現状 LOOK が欠落し対象が誤って FIELD/BOUNCE になる
     （`parse_ability` の「見て」「公開し手札に加える」「残り」の3分割が崩れる）。
     正しくは LOOK(deck→temp) → MOVE_CARD(temp→hand, フィルタ) → DECK_BOTTOM(残り)。
     `parser.py` の構造分解の見直しが必要（リスク中〜大）。
   - **構造断片「自分の手札1枚を」（11件 OTHER）** — 同じくセグメント分割で動詞が切り離される。
   - **ライフ look-and-place「ライフの上から1枚を見て、ライフの上か下に置く」（10件）** —
     上下選択 UI が未実装。
