...
| 指標 | 開始時 | 現在 |
|---|---|---|
| 原子句カバレッジ（ルール命中率） | 0% | 約64.5%（grant_keyword + ライフ操作5種） |
| `ActionType.OTHER`（実行時に何もしない句） | 942 | 368（約61%減） |
| テスト総数 | 17 | 55（全緑） |
...
> のエンジン実行も追加。詳細は `docs/parser_v2.md`。
...
1. **裾野の OTHER（約368件）のルール化** — 頻度は低く多様（上位でも10件前後/表現）。
   `effect_diagnostics.py` の「OTHER化する原子句ランキング」を起点に継続。
   候補: デッキ並び替え（デッキの上か下に置く）、可変ドン返却、ライフを見て上か下に
   置く（look-and-place）等。
