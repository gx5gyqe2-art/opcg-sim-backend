...
| `ActionType.OTHER`（実行時に何もしない句） | 942 | **12** |
...
- **残12件（複合構造・トリガー断片・記号）**:
  - ライフ全見て1枚をデッキ上へ（2枚 ST13-004 等）: LOOK_LIFE→選択→DECK_TOP の多段。要 parser/エンジン
  - dual-tier 選択（トラッシュのコスト4以下と2以下を1枚ずつ）/ トラッシュ枚数で段階効果（各1枚）: 多段選択・閾値分岐
  - トリガー/条件断片（「自分のリーダーが「しらほし」の場合」「イベントか【ブロッカー】を発動した時」）: 構造分解側の課題
  - サーチ結果をライフへ（「カード1枚までを、ライフの上に加える」= TEMP→LIFE）
  - 記号断片 ①/➀（OP07-097 の DECLARE_COST マーカー等。実害なし）
  - 「代わりに〜できる」任意置換（1枚）: C(E14/E15) と同系統で見送り中
