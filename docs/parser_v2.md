...
| 指標 | 値 |
|---|---|
| 原子句カバレッジ（ルール命中率） | **99%超** |
| `ActionType.OTHER`（実行時に何もしない句） | **6** |
| atoms.py ルール数 | **83** |
| ゴールデンコーパス | **~152件** |
| テスト総数 | **284（全緑）** |
| レガシー vs V2 退行 | **0** |

## atoms.py ルール一覧（83ルール）
...
> **さらに追加した主なルール/メカニクス**（最新）:
> `deal_damage`/`look_opp_deck`/`reveal_deck_top`/`order_life`/`execute_event`/`don_rest_cost_fragment`/
> `declare_victory`(+VICTORY engine)/`move_attached_don`(+engine)/`redirect_attack`(+engine)/
> `rested_play_passive`・`no_effect_play_passive`(+engine フック)/`search_to_life`/`life_to_deck_top`/
> `don_cost_circled`(丸数字コスト→REST_DON)。共有対象二択 `_parse_shared_target_choice`、
> コスト節先頭条件の引き上げ `_extract_leading_condition` も parser に追加。
>
> **ミスターゲット是正（2026-06-11）**: `deck_bottom_general` の「持ち主」OPPONENT 補正が
> 「自分の」明示を無視していた点、`parse_target` が期間句「次の相手のターン終了時まで」の
> 「相手の」を対象側と誤認していた点、`don_attach` の付与先フィルタ脱落を修正。隠しゾーンからの
> 「見て選ぶ」は `TargetQuery.flags="REVEAL_SELECT"` で対話選択に切替（`life_to_deck_top`）。
> 検査は `tests/interactive_target_audit.py`。
