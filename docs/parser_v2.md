...
- **条件の fail-safe**: 解釈不能な `OTHER` 条件は False（誤発動防止）。`GENERIC`（未分類だが
  実在する条件）は誤抑制回避のため許容＋ログ。リーダー特徴の `『X』` 記法を `LEADER_TRAIT`
  に分類して評価可能化。
...
- `ActionType.OTHER`（実行時に何もしない句）: **942 → 342 に削減（約64%減）**
  （ライフ表/裏向き・ドンデッキへ戻す 等が OTHER から実行可能 ActionType へ。なお
  キーワード付与は従来 OTHER ではなく誤 BUFF だったため、146 句の
  `BUFF`→`GRANT_KEYWORD` は別途の改善）
- テスト: `test_parser.py`(8) + `test_golden.py`(35) + `test_effects_engine.py`(23)
  + `test_gameplay_smoke.py`(2) = 全68件緑
