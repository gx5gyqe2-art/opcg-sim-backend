...
| ライフ操作 | `life_recover` / `life_to_hand` / `hand_to_life` / `life_to_trash` / `life_cards_to_trash` / `life_scry_top` / `life_face` / `life_to_deck_top` / `order_life` / `reveal_own_life_top` / `search_to_life` |
