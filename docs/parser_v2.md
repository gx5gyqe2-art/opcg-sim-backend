...
- **「お互い」連結効果の構造分解**: 「（前段）…その後、お互いは手札がN枚になるように、
  自身の手札を捨てる」（OP05-058）等で、能力分割や「N枚になるように」の枚数解釈が
  不安定な場合がある。`Player.ALL` の単一対象選択としてモデル化されるため、両プレイヤーへ
  独立に適用する「同時両側」処理は未対応（→ `SPEC.md §6.1`）。バグ確定時は修正のうえ
  `tests/test_verified_decks.py` に回帰アサートを追加する（`TEST_SPEC.md §8`）。
