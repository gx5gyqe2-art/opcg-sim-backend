...
## 現況（ルール38種 + エンジン/resolver/継続効果/除去保護/置換効果 時点）

- 原子句カバレッジ（ルール命中率）: **約87.1%**
- `ActionType.OTHER`（実行時に何もしない句）: **281**（開始時 942 から約70%削減）
- テスト: `test_parser.py`(8) + `test_golden.py`(57) + `test_effects_engine.py`(40)
  + `test_gameplay_smoke.py`(2) = 全107件緑
...
- パーサルール: draw / ko / rest / rest_self_cost / power_buff / discard /
  ...
  **bounce / deck_bottom_general / remaining_deck_top_or_bottom / play_card_from_zone /
  active_target / blocker_disable / rush_natural**（＋ rest/mill_deck を拡張）
