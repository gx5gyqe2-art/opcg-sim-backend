...
- `GRANT_KEYWORD`: `status` のキーワードを対象の `current_keywords` に付与
...
## 現況（ルール21種 + エンジン/resolver/継続効果/除去保護 時点）

- 原子句カバレッジ（ルール命中率）: **約64.5%**（grant_keyword 57→60.1%、
  ライフ操作5種 60.1→64.5%）
- `ActionType.OTHER`（実行時に何もしない句）: **942 → 368 に削減（約61%減）**
  （ライフ表/裏向き等が OTHER から FACE_UP_LIFE 等へ。なおキーワード付与は従来
  OTHER ではなく誤 BUFF だったため、146 句の `BUFF`→`GRANT_KEYWORD` は別途の改善）
- テスト: `test_parser.py`(8) + `test_golden.py`(29) + `test_effects_engine.py`(16)
  + `test_gameplay_smoke.py`(2) = 全55件緑
