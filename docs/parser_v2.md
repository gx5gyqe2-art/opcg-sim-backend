...
- **`execute_main`（EXECUTE_MAIN_EFFECT）の2形態**: (1) target 無し＝発生源自身の参照トリガー
  （【メイン】/【登場時】等）を再展開（`_expand_main_effect`）。(2) **target 付き**＝「（自分の）
  トラッシュにある…イベント…の【メイン】効果を発動する」（EB03-031）。選択句を `parse_target`
  で対象化（zone=TRASH・card_type=EVENT・cost_max・`is_up_to`）し、resolver が選んだイベントを
  **source として** `resolve_ability` で発動する（`_execute_selected_main`。「このカード」参照が
...
- **「お互い」連結効果の構造分解**: 「お互いは手札がN枚になるように、自身の手札を捨てる」
  （OP05-058）の「N枚になるように」は `discard` ルールが `DOWN_TO_N` として解釈する（従来は
  literal の N 枚と誤読。単独の OP14-054 も同時に修正）。「お互い」は `matcher` が
  `Player.ALL` ＋ `BOTH_SIDES` フラグとして解析し、`resolver` が両プレイヤーへ独立に同時適用する
  （→ `SPEC.md §6.1`。残る制約＝選択を伴う両側効果の完全な同時対話化は未実装）。
- フォールバック（レガシー parser へ落ちた未対応原子句）は `EffectParserV2.unmatched` に
  蓄積され、`tests/test_quality_gates.py::test_parser_fallback_ratchet`（上限0）で監視する。
  新規の未対応表現はここで検知し、`atoms.py` にルールを追加して解消する。
