...
## 現況（ルール30種 + エンジン/resolver/継続効果/除去保護/置換効果 時点）

- 原子句カバレッジ（ルール命中率）: **約75.2%**（grant_keyword 57→60.1%、
  ライフ操作5種 60.1→64.5%、ドン操作4種 64.5→70.4%、
  自己トラッシュ/自己アクティブ/ステージのレスト/mill/残り→トラッシュ 70.4→75.2%）
- `ActionType.OTHER`（実行時に何もしない句）: **333**（横ばい）。今回追加の5表現は
  レガシーでは既に非 OTHER に落ちており OTHER 指標には現れない。実利はカバレッジ向上・
  自己対象の正確化（SOURCE）・`TRASH_FROM_DECK` 実行系の新設（mill のサイレント no-op 解消）。
- テスト: `test_parser.py`(8) + `test_golden.py`(45) + `test_effects_engine.py`(36)
  + `test_gameplay_smoke.py`(2) = 全91件緑
...
- パーサルール: draw / ko / rest / rest_self_cost / power_buff / discard /
  ...
  **trash_self / active_self / mill_deck / remaining_trash**（＋ rest を「できる」対応へ拡張）
