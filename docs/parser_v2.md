...
対応 kind: `POWER`（timed_power）/ `FLAG`（timed_flags）/ `KEYWORD`（timed_keywords）。
...
- テスト: `test_parser.py`(8) + `test_golden.py`(38) + `test_effects_engine.py`(27)
  + `test_gameplay_smoke.py`(2) = 全75件緑
