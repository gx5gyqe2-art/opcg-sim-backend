...
| PR | 内容 | 主な成果物 |
|---|---|---|
| **PR1: 基盤＋自己対戦ランナー** ✅実装済 | §2.1 リファクタ・`clone()`・`get_legal_actions`・**決定論ランナー（§4.1/4.2）＋インバリアント検出（§4.3）** | これだけで効果検証ツールとして機能し、以降の回帰を即検出できる |
| **PR2: AI（backend）** ✅実装済 | 評価関数（§2.3）・探索（§2.4）・難易度（§2.5）・`/api/game/cpu/step`・create フラグ（§2.6） | 強い CPU |
| **PR3: frontend** ✅実装済 | メニュー・RealGame `vsCpu`・ポーリング駆動・client / 型（§3） | プレイ可能な CPU 対戦（`opcg-sim-frontend/docs/CPU_BATTLE_PLAN.md`） |
| **PR4: 仕上げ** ✅実装済 | `docs/SPEC.md`（両リポジトリ）と feature 分類の更新 | 完成。難易度チューニング・オラクル接続（§4.4）は将来の継続課題 |
...
| 成果物 | パス |
|---|---|
| 共通アクション適用（HTTP/AI/自己対戦の単一コアパス） | `opcg_sim/src/core/action_api.py`（`apply_game_action`/`apply_battle_action`） |
| 状態複製・合法手列挙・既定対話解決 | `opcg_sim/src/core/gamestate.py`（`GameManager.clone`/`get_legal_actions`/`default_interaction_payload`） |
| 対局中インバリアント検出 | `opcg_sim/src/core/invariants.py`（`check_invariants`/`check_turn_boundary`） |
| 決定論的自己対戦ランナー（CLI＋JSONL トレース） | `tests/cpu_selfplay.py` |
| スモーク/不変条件テスト | `tests/test_cpu_selfplay.py` |
| HTTP エンドポイントの委譲 | `opcg_sim/api/app.py`（`/api/game/action`・`/api/game/battle` を action_api へ委譲） |

検証: 既存スイート `724 passed, 1 skipped, 2 xfailed`、`full_card_audit.py` の
EXCEPTION/CARD_LOSS/TEMP_LEAK=0、自己対戦 300+ ゲードがインバリアント違反なく完走（決定論再現を確認）。

### PR2 実装メモ（完了）

| 成果物 | パス |
|---|---|
| CPU AI（評価関数・1-ply 先読み・難易度・暴走防止ガード） | `opcg_sim/src/core/cpu_ai.py`（`evaluate`/`decide`/`decide_guarded`） |
| CPU 対戦の生成フラグ・メタ登録・ポーリング | `opcg_sim/api/app.py`（`game_create` の `vs_cpu`/`cpu_difficulty`/`cpu_deck`、`CPU_GAMES`、`POST /api/game/cpu/step`） |
| 自己対戦ランナーへの AI 方策統合（効果トレース用） | `tests/cpu_selfplay.py`（`--policy ai --difficulty hard`） |
| AI/エンドポイントのテスト | `tests/test_cpu_ai.py` |

- 難易度: `easy`=ランダム / `normal`=貪欲 1-ply / `hard`=貪欲 1-ply（評価関数で採点）。
- `/api/game/cpu/step` は CPU(p2) の次の 1 手を適用し `{cpu_acted, cpu_event, waiting_for}` を返す
  （`waiting_for`: `cpu`=続けて step / `human`/`human_decision` / `game_over`）。
- 暴走防止: ターン内の手総数キャップ（60）と起動効果/ドン付与の繰り返しキャップ（3）で、
  per-turn 制限の無いカード（例: イガラム）でも CPU ターンが必ず終わる。これは効果側の
  per-turn 制限付け忘れの検出ポイントでもある（自己対戦トレースで顕在化）。
- 検証: 既存スイート `730 passed, 1 skipped, 2 xfailed`、`full_card_audit.py` ゲート 0、
  AI 対 AI（hard）自己対戦が完走、hard が easy に 9-0。
