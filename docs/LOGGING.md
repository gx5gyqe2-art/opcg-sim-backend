...
実アプリ対局（opt-in）:
  POST /api/game/create {cpu_trace:true, seed?}
    └─ CPU_GAMES[gid] にメモリ蓄積（seed / 操作列 / 各意思決定の思考トレース / 盤面フレーム）
  GET  /api/game/{game_id}/replay
    └─ {replay: 種(schema/seed/leaders/decks/difficulty/actions), decisions:[...]}
  GET  /api/game/{game_id}/replay/frames
    └─ 上記 + frames:[...]（リプレイビューア用の一括ペイロード）
```
...
traced 対局では、**各アクション適用後の盤面スナップショット**（＋create 直後の初期盤面＝フレーム0）を
`meta["frames"]` に蓄積する（`services/replay.py::_replay_record_frame`）。対局中の効果シャッフル等が
global RNG を消費するため、記述子からの注入再生では盤面を正確に再構成できない（decide 再実行が要る）——
**記録時スナップショットは再計算ゼロ・分岐リスクゼロ**でこれを回避する設計。

- フレームのカードは**動的状態のみ**（uuid/card_id/name/power/cost/is_rest/is_face_up/attached_don/
  keywords/ability_disabled/is_frozen）。効果テキスト等のマスター情報は card_id からフロントの
  カード DB で引く＝フレーム総量を約 1/5 に抑える。
- 整合はインデックスで明示する: `frames[i].action_index`＝直前の `actions` インデックス（フレーム0のみ
  None）、`decisions[k].action_index`＝その決定が指した `actions` インデックス。
- メモリ保険: `_FRAME_CAP`（4000）到達で記録を止め `frames_truncated` を立てる（対局長自体は
  `TURN_ACTION_CAP` で有界）。
- 取得: `GET /api/game/{game_id}/replay/frames`（`/replay` の内容＋`frames`＋`frames_truncated`）。

回帰は `tests/test_replay_frames.py`（フレーム整合・コンパクト形・キャップ）が固定する。
