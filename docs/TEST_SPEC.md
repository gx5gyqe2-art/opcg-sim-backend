...
| `tests/scripts/exit_head_probe.py` | **出口ヘッド候補の安価な足切り**（v41・読み取り専用）: MCTS を回さず2列だけ測る——(1) 各検証点の合法手を `resolved_branch_values`（＝実対局の防御窓読み出しそのもの）で並べ、argmax が裁定済み accept に入るか（gap 列＝枝間マージン）、(2) 教師コーパス盤面での `predict_exit(kind) − predict()` の平均/標準偏差（平均が支配的＝一律バイアスを学んだ／標準偏差が支配的＝盤面ごとの差を学んだ）。**実測 2026-08-07**: gen12 の枝間マージンは 0.02〜0.03 なのに defcf（584群775ペア）で学習したヘッドの摂動は標準偏差 0.23〜0.27＝約10倍で、m1@14 を直す腕は必ず m2@44 を壊した（5腕すべて 4/8）。腕を増やす前にこの2列でコーパスの信号がマージンを超えているかを判定する。注意＝戦闘窓でない点（m1@3/m4@2/m2@66）の行は「root を戦闘箱の規約で並べたら」の近似で実対局の decide とは経路が違う（一次情報は m1@14/m1@15/m2@58） |
| `tests/scripts/label_reliability.py` | **教師ラベルの信頼度測定**（v44・読み取り専用・**本生成前の足切り**）: `plan_cf_gen` が出す分割半ラベル（world を偶奇で割った独立2組＝`value_a`/`value_b`）から、(1) 半々の順位一致率（0.5=コインフリップ＝ラベルはノイズのみ）、(2) **δ選抜後の一致率**＝学習が実際に教わる順位の正答率（勝者の呪いがあると選抜後の方が下がる）、(3) 推定ラベル1σ と枝間マージン（gen12 実測 0.02〜0.03）の比、を出す。**なぜ**＝コーパス生成→学習→ゲート→アリーナの1周は1〜2日かかり、v41 は「学習しても直らない」で1周、v42 は「アリーナ有意退行」で568局を溶かした。どちらも教師の信頼度を先に測れば着手前に分かった。worlds を増やす投資の判断根拠に使う。分割半は worlds/2 の予算で測るため数字は**保守側**（本番ラベルは全世界を使う） |
