...
| `tests/test_action_feats_v2.py` | **行動特徴 v9 拡張＋幅互換層**（`action.py`/`policy.py`・PR#188 レビュー#7・必須/標準）: カウンター値（0/1000/2000→0/0.5/1.0）と対象=リーダー flag の append-only 追加（@82 型「カウンター温存」を policy が吸収する素地・1.9k教師で支持一致 60→62% 頭打ちの実測が根拠）・**旧次元 net × 新次元行列＝末尾切詰で出力完全一致**（既定 gen5 の serve 挙動不変の防壁）・`extend_action_dim`（零行温スタート＝恒等→旧22次元記録のゼロ埋め混在で学習が回り新特徴に勾配が流れる） |
...
| `tests/test_referee_labeler.py` | **基盤健全性**（`cpu_infra`）。**レフェリー再ラベルの純ロジック**（v9 フェーズ1・`referee_labeler.py`・純関数のみ＝高速）: 採掘候補の選抜（sat/blind ⌈K/2⌉クォータ＝blind 飢餓防止・不足時の相互埋め戻し・隣接間引き・上限・index昇順）・policy 教師の構築（同価値バンド上位プランの初手 multi-hot＝バンド外は 0・同一初手の合算・初手が合法手に無ければ None＝誤教師を作らない） |
| `tests/test_coach_gate.py` | **基盤健全性**（`cpu_infra`）。**コーチゲートの判定則**（`coach_gate.py`・純関数）: hit の card/type-only 判定・judge の非退行（確実点の大幅落ちのみ退行・不確実点の揺れは不問）と改善（ヒット計比較）・VERIFIED 採録の整形（非空 accept・実在ゲーム） |
...
| `tests/scripts/coach_gate.py` | **コーチゲート**（v9 §4・mark_gate の後継・ユーザ承認 2026-07-18）: レフェリー検証済み決定点（真盤面・出典明記の VERIFIED 採録）の**同価値バンド初手集合**への所属で gen5 と候補を同条件 decide 比較（非退行=base≥0.8 の点で chall≤base−0.4 が無い／改善=ヒット計≥base計・`COACH_GATE_RESULT`）。人間マーク述語の mark_gate は真実源と矛盾する点が実測された（@115=守るのが唯一の勝ち筋・@33=全攻撃同価値）ため、人間マークをレフェリーで裏取りした形に昇格 |
...
| `tests/scripts/referee_labeler.py` | **レフェリー再ラベル・パイプライン**（v9 フェーズ1・`docs/cpu_v9_plan.md` §2）: ①両席 learned(gen5) の記録つき自己対戦（`record_selfplay_descriptor`＝decide の rng 隔離で scripted 再生可能）→②採掘（同一パスの読み取り専用 observer・**miner v2**: 効率盲点＝**上位M(4)手**の1-ply後 value spread<ε（全手 spread は明確な悪手混入で@64型を取り逃す）／飽和負け＝value<しきい・**sat/blind ⌈K/2⌉クォータ**＝敗者側終盤の sat 洪水で blind が飢える構造の防止・1局上限K点）→③真盤面再生（**採掘時の actor/pend 種と照合**＝index ズレで別局面を黙ってラベルする事故の防止）＋プラン自動列挙＋CRN 対照評価（飽和は捲りエスカレーション）→ policy 教師＝バンド上位プラン初手 multi-hot・value 教師＝z=2·wr−1（捲り率）。出力=バッチスキーマ v2＋meta(source="referee_label"・miner版数。worker はワーカー運用時に w1 等へ上書き)＝ pd_learn 直結（staleness は source で免除）。教師ネット gen5 固定＝外部の錨 |
| `tests/scripts/ref_finetune_smoke.py` | **v9 フェーズ2 スモーク**（`docs/cpu_v9_plan.md` §3 の当たり付け・読み取り専用）: v9-label 全枝の教師バッチを収集→決定単位 train/val 分割（固定seed）→gen5 温スタートで value/policy 微調整（LR 候補比較・policy-smooth 床＝未評価ハードゼロの緩和）→前後評価（val の policy 支持一致率・KL・value MAE/corr）。同梱ネットは書き換えない（--out 指定時のみ候補 npz 保存＝後続ゲート運転用） |
