...
| `tests/test_referee_labeler.py` | **基盤健全性**（`cpu_infra`）。**レフェリー再ラベルの純ロジック**（v9 フェーズ1・`referee_labeler.py`・純関数のみ＝高速）: 採掘候補の選抜（飽和負け優先・隣接間引き・上限・index昇順）・policy 教師の構築（同価値バンド上位プランの初手 multi-hot＝バンド外は 0・同一初手の合算・初手が合法手に無ければ None＝誤教師を作らない） |
...
| `tests/scripts/referee_labeler.py` | **レフェリー再ラベル・パイプライン**（v9 フェーズ1・`docs/cpu_v9_plan.md` §2）: ①両席 learned(gen5) の記録つき自己対戦（`record_selfplay_descriptor`＝decide の rng 隔離で scripted 再生可能）→②採掘（同一パスの読み取り専用 observer: 効率盲点＝合法手の1-ply後 value 差<ε／飽和負け＝value<しきい・1局上限K点）→③真盤面再生＋プラン自動列挙＋CRN 対照評価（飽和は捲りエスカレーション）→ policy 教師＝バンド上位プラン初手 multi-hot・value 教師＝z=2·wr−1（捲り率）。出力=バッチスキーマ v2＋meta(worker="ref")＝ pd_learn 直結。教師ネット gen5 固定＝外部の錨 |
