...
| `tests/test_guard_policy.py` | **基盤健全性**（`cpu_infra`）。**受け方針箱 P6-c の契約**: `shape_moves` は SELECT_COUNTER/SELECT_BLOCKER にだけ作用（pass=両方落とす・minimal=最小印字1枚＋素通し・hold=総量で守れる純カウンター窓だけ PASS を落とす・イベント/起動/混在窓は不変・空にしない）／`select_guard_policy` は実盤面で4方針の台本比較を返す（m1@14 実測: hold −0.585 vs 他 −0.281＝守るのが損な入口で死守が正しく最下位）／エンジン統合は方針経由でも従来の合法手を返す・`SERVE_GUARD_POLICY` 既定 OFF |
| `tests/test_plan_box.py` | **基盤健全性**（`cpu_infra`）。**マクロ手化 P6-a＝ターン箱（プラン×箱語彙）の契約**: 箱化フルセットのエンジンで `select_plan` の提案が**箱の列**として立つ（m2@44 で DON_BOX が sig に現れる・候補2本以上）／プラン読み出しの実対局出力は常に原始手（DON_BOX 素通し事故の再発ガード・6シードで採用実績も確認）／`execute_plan(dialog_box=True)` が空プランでもターン末出口を返す |
...
| `tests/scripts/defense_audit_probe.py` | **防御監査 P4-a＝防御窓の結合判断の質の実測**（2026-08-24・読み取り専用）: 自己対戦の被攻撃窓（SELECT_BLOCKER/SELECT_COUNTER 入口）で結合防御の変種（素通し／守り切る最小の札組／+1000 余裕・lethal 防御台本と同じ印字値算術）を台本適用→戦闘出口 value で順位づけし、エンジン実選択（SERVE_BATTLE_COMMIT のコミット済みプラン）との乖離率と型（過少/過剰防御）を出す。マクロ手化 P4（防御箱＝要点総量の候補化）の設計根拠となる入口計測。教師には使わない |
