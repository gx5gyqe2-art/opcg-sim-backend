...
| `tests/test_flagship_link.py` | 収集の蓄積と開催紐付け（`/collect`・`/link/review`・`/link/approve`、設計 docs/design.md §16.7）。収集→DB蓄積／未紐付けポストの TCG+開催への照合レビュー（handle自動候補・個人ポストは候補ゼロ）／一括承認保存で未紐付けから消える／TCG+エラー502・未設定503。検索と TCG+（`tcgplus.py`）は monkeypatch 遮断・SQLite(tmp) 永続 |
