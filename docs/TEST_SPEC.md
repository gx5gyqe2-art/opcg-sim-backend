...
| `tests/test_search_averse_probe.py` | **基盤健全性**（`cpu_infra`）。**SEARCH_AVERSE 追跡の判定則**（`tests/scripts/search_averse_probe.py`・v21・純関数 `diagnose`/`world_sensitive`）: **アブレーションは base を上回って初めて原因**（base と同値の argmaxN/減衰off/auxoff を『読み出しのせい』と誤診しない＝2026-07-29 に実際に踏んだ形）・base≥0.5 は NOT_FAILING@deep・SEARCH_Q_BOUND は全深さで accept の Q が下回ることを要求・世界依存フラグは 0<base<1 のみ（0/1 では立てない）。実探索は回さず高速固定 |
...
| `tests/test_mcts_turn_quiesce.py` | **基盤健全性**（`cpu_infra`）。**ターン静止**（`TreeMCTS._leaf_value`＋`resolve_turn_inplace`・v37①・2026-08-06）: root 手番側の自ターン途中の葉をターン末まで進めてから評価する機構（メイン手=policy最良・戦闘窓=出口valueの箱）。固定＝延長はターン境界で停止（評価盤面はターン所有者交代済み）・相手ターンの防御窓では延長しない（相手の動きは決め打ちせず分布と再計画で扱う範囲限定）・OFF は従来同一・副作用ゼロ（盤面/乱数復元）。**判定は中立**（8点ゲート 7.06 で全点本番同一・decide +46%）＝既定 OFF のまま採用見送り（箱化済みの木では葉が実質ターン境界に近い）。機構はロールバック可能な負の結果として保存 |
| `tests/test_turn_plan.py` | **基盤健全性**（`cpu_infra`）。**プラン読み出し**（`opcg_sim/src/learned/plan.py`＋`LearnedEngine._plan_step`・v37②・2026-08-06）: 自ターンのメイン判断を「提案（policy温度サンプル・1本目=argmax・重複除去）→K世界CRNの自ターン末value平均→ターン内sticky実行（非合法化は縮退・尽きたらTURN_END・割れたら1回再計画）」で決める機構。固定＝move_sig の純粋性/衝突なし・**プランの手が別の決定化世界でもそのまま合法**（決定化は相手伏せ手札のみ＝実行可能性の核）・select_plan は score argmax（診断dictと整合）・ON で合法手＋キャッシュ・OFF は既定経路と同一手・壊れたプランは縮退してクラッシュしない。**判定 FAIL**（m1@3 1.00→0.25＝ターン末valueのバイアスはK世界平均で消えない）＝ターン出口教師が揃うまで既定 OFF |
...
| `tests/test_exit_heads.py` | **基盤健全性**（`cpu_infra`）。**出口専用 value ヘッド**（`ValueNet.EXIT_HEADS`／`enable_exit_head`／`predict_exit`／`backward_exit`・v39 ターン末 We1..be2 / v41 戦闘出口 Wb1..bb2）: 箱の階層ごとに較正を分ける出力の append-only 拡張。全性質を階層で parametrize＝**新しい箱を足すときの雛形**。動機は2つの負の結果——v38（ターン出口教師を既存ヘッドへ同居させると m5@7/m2@66 は満点になるが m1@15 が 1.00→0.00・8点合計 3.06 < 本番 3.44・α補間でも救えない＝gen12 の m1@15 のマージンが +0.062 しかない）と v40（防御CFで**本体 value を直接**順位学習するとコーチ 8.00/8.00 満点なのにアリーナ 0.447 CI[0.409,0.485]・284ペア＝有意な退行。全面学習は盤面評価そのものを全域で動かす）。固定＝有効化は残差ゼロで**恒等**・未有効/旧 npz は `predict_exit` が既存ヘッドへフォールバック（同梱ネットは無改修）・ヘッド学習は胴体/既存ヘッド/補助ヘッド/**他階層のヘッド**を **bit 不変**に保つ・順位ヒンジがその出口の順位を実際に改善・save/load と複製（expanded/widened）でヘッドが残る・消費側の結線（ヘッド無しネットでは v39/v41 以前と同計算／`evaluate_plan` は出口盤面だけをターン末ヘッドで測り戦闘窓は戦闘ヘッドへ渡す／`_battle_window_choice` が箱へ渡す物差しが戦闘出口ヘッドである＝**この引数が本体 value に戻ると v41 は無効化される**）・**リソースヘッド**（`in_cols`・2026-08-14 ユーザ提案「手札/盤面/ライフの束で交換レートを学ぶ」）＝胴体 A1 でなく生 scalars の指定列を読む変種でも同性質が成立（恒等・胴体凍結・save/load 往復・`expanded` 挿入時の列番号追随＝恒等保存）＋`E.battle_resource_cols` の列健全性（範囲内・重複なし・世代間 append-only） |
| `tests/test_battle_commit.py` | **基盤健全性**（`cpu_infra`）。**入口コミット**（`SERVE_BATTLE_COMMIT`／`LearnedEngine._battle_commit_step`・2026-08-15 ユーザ決定「人間はカウンターを1枚切った後に考え直さない」）: 防御窓のプランを入口で1回立て以後は実行のみにする。固定＝**既定 ON**（アリーナ同等 wr0.520 CI[0.451,0.589]×31%高速・`SERVE_BATTLE_COMMIT=False` で gen14 出荷時と同一挙動＝ロールバック経路）・**入口の1手はステップ読み出しと同一**（同じ決定化/同じ `resolved_branch_values`/同じ argmax＝「何を選ぶか」は不変で「いつ決めるか」だけが変わる）・プラン立ての**副作用ゼロ**（盤面と global random を復元）・2手目以降は**キャッシュの実行**（再計画しない）・プランが実盤面と割れたら**1回だけ**立て直し失敗時は従来経路へ退化。初回A/Bの大敗 0.320 は採取側が `box_depth=0`（policy最良→PASS の弱い選択器）で継続を決めていた実装バグで、**評価と実行の継続規約を一致**させて解消（`docs/reports/battle_commit_20260815.md`） |
| `tests/test_mcts_box_battle.py` | **基盤健全性**（`cpu_infra`）。**木の中の箱化**（`TreeMCTS._expand` の戦闘窓畳み込み・v35・2026-08-05・ユーザ指摘）: 実対局の窓を出口 value で選ぶようにしても（`SERVE_BATTLE_READOUT`）**木の中の戦闘窓は通常ノードのまま**訪問を配っており、同じ場面を木と実対局で違う規約で扱う唯一のずれが残っていた。二人零和では相手は最善応手を返すのが正しく、PUCT の訪問混合は**収束前の副産物**であって設計された保険ではない＝畳む方がミニマックスに近い（幅も失われない。木には別の攻撃順・別盤面の戦闘が無数にあり各々が独立した箱を持つ）。副次効果＝カウンターの組合せに配っていた訪問がメイン判断へ回る／**攻撃の帰結が具体的な出口として立ち上がる**（「相手手札−1＝カウンターを絞り出した」か「相手ライフ−1・手札+1＝通した」の二択。攻撃は必ず相手に損失を強いるので『止められる＝無駄』ではない）。固定＝戦闘窓ノードが**単一辺**へ畳まれその辺が出口最良（m1@15 で止まる 2000 カウンター）・葉見積もりも同じ出口の値（木と読み出しの規約一致）・OFF は全合法手を子に持つ（ロールバック可能）・メインフェーズは畳まない・探索後も盤面と global random が復元。**gen12 で既定 ON**（m2@44 0.00→0.62・m5@7 0.00→0.56 を初めて動かした） |
| `tests/test_battle_readout.py` | **基盤健全性**（`cpu_infra`）。**戦闘窓の読み出し**（`cpu_learned._battle_window_choice`／`mcts.resolved_branch_values`・v35・2026-08-05・ユーザ整理「バトルを一つの箱としてまとめて、探索と value はバトルをするかどうか／バトルの結果どんな盤面・手札・ライフになるかのみを判断する」）: カウンター/ブロッカー選択は**箱の出口**で決まる局所判断なのに full-tree の root Q は箱の外の深い未来まで平均して判断を薄める（実測: 出口評価は防御3類型 m1@14/m1@15/m2@58 をすべて正しく順序づけるのに探索後 Q では逆転。木の葉の68%が「次の自ターン」の通常盤面＝旧レートに引き戻すため。蒸留アンカーの層別化で尺度を動かす方向は4設計とも自ターン判断 m1@3 を壊すことを実測済み）。**判断するのは葉評価と同じ value ネット自身**で、別系統の防御ロジックではない。固定＝出口で順序づくこと（止まるカウンター > 止まらないカウンター・ネット非依存の純関数評価器で検査）・**副作用ゼロ**（盤面と global random が完全復元）・**value を反転すると選択も入れ替わる**（決めているのが value であることの検査）・戦闘中でないメインフェーズは読み出しを通らない。**gen12（2026-08-05）で既定 ON**（解決後盤面で学習した value と対で効く。gen11 の出口評価は m1@15 で PASS が最上位のままだった＝重みと機構はペア）。テストは**明示 OFF が旧挙動と一致**することでロールバック経路を固定する |
...
| `tests/test_plan_freeatk_gen.py` | **基盤健全性**（`cpu_infra`）。**V8 フリー攻撃族生成器の契約**（`tests/scripts/plan_freeatk_gen.py`）: `free_kill_target` は自リーダーがアクティブ・相手に「レスト済み・パワー≤リーダー」のキャラ・相手にアクティブなブロッカーなし、の時だけ（リーダー, 最大の獲物）を返す（レスト済みブロッカーは阻止不能＝対象になる）／`plan._attack_candidates` は攻撃者 uuid で絞り、対象 uuid 指定時（3要素形）は `target_ids[0]` も一致させる |
| `tests/test_plan_sticky.py` | **基盤健全性**（`cpu_infra`）。**プラン読み出しのターン内 sticky 鍵**（W4・2026-08-20）: 鍵は `id(manager)` でなく `_plan_sticky_id`（初見時にトークン付与・clone=deepcopy が継承）＝in-place 進行（実対局）とクローン進行（リプレイ/ロールアウト）のどちらでも「ターンに1回立案・以後は列を1手ずつ」／ターンが替われば再立案／別対局は共有しない／プランが尽きたら TURN_END で閉じる |
...
| `tests/test_arena_resume.py` | **基盤健全性**（`cpu_infra`）。**再開可能アリーナ**（`tests/scripts/arena_resume.py`・v25）: 台帳 jsonl の読み戻し＋残り seed 抽出（計画順・重複なし＝10分制限下の分割実行の土台）・最終判定は全ペア消化後にのみ出る（部分結果で promoted を出さない）・判定規約（勝ち数0..2→ペア水準0/0.5/1 正規化・wr≥frac かつ CI下限>0.50）が arena_gate と同値。実対局は回さず高速固定。**`--cand-box`（v35）**＝候補席にだけ戦闘窓の箱読み出し＋静止探索を与える A/B（席別 seam の検査は `test_promotion_gate.py`） |
...
| `tests/scripts/plan_freeatk_gen.py` | **ターン出口教師・系統3＝V8 フリー攻撃族**（π非依存・2026-08-22）: 段3裁定 504004@43（有利な攻撃を残して TURN_END）の一般化。自リーダーがアクティブで相手に「レスト済み・パワー≤リーダー」のキャラがいて相手にアクティブなブロッカーがいない決定点で、「リーダーでその獲物を取る→閉幕」（+0.5）vs「素通し閉幕」（−0.5）の論理支配ペア（リーダーは戦闘で失われず・レストに守備上の失点なし・カウンターで守られても相手の手札を削る）。実現は `plan.scripted_plan` の ATTACK 意図対象指定（本族のため追加した後方互換の3要素形）。出力 `planatk_*.npz`（plandom 互換） |
...
| `tests/scripts/arena_resume.py` | **再開可能アリーナ**（v25・`arena_gate.py` の chunk 実行版）: 実行環境がフォアグラウンド約10分制限＋ターン終了でバックグラウンド回収のため、800局判定を一度に走り切れない。ペアスコアを jsonl 台帳へ追記し、再実行のたびに未消化 seed から `--max-pairs` ぶん進める＝10分×N回で同一判定を積み上げる。帯設計は `arena_gate.plan_bands`・対局は `promotion_gate._play_pair`・集計は `arena_parallel._pair_level_ci` を import（判定規約の二重化なし）。全ペア消化後の実行が `ARENA_RESUME_FINAL` を出す。**--cand-plan-readout**（2026-08-22）＝候補席だけプラン読み出し（v37②）を有効化（env の SERVE_PLAN_READOUT は両席に効いて対照を汚すため使わない）。**void（2026-08-16）**: 対局がエンジン欠陥で成立しなかったペア（上限手数 MAX_STEPS 等）は score=null で台帳に残し、消化済みとして数えつつ勝率の母数からは外す＝1ペアの失敗でシャードを落とさない。結果には `void` 件数を必ず載せ、全ペア void なら判定を出さない |
...
| `tests/scripts/search_averse_probe.py` | **SEARCH_AVERSE 追跡プローブ**（v21・読み取り専用・`docs/reports/cpu_v21_search_averse_20260729.md`）: v20 が分離した「prior も 1手先 value も支持するのに探索が選ばない」点について、root の Q/N を sims 昇順にトレースし、**4アブレーション**（argmax(N) 読み出し／終局減衰 off／aux off／多世界(8)平均）で犯人を切り分ける。判定則は**アブレーションが base を上回って初めて原因**（≥0.5 だけだと base の高い点を誤診）＋**世界依存フラグ**（0<base<1）。**実測(gen8): 容疑者3つ（読み出し/減衰/aux）は全点 base と完全一致＝無罪**・真の SEARCH_Q_BOUND は m4@12 のみ（sims↑で accept の訪問が 0.28→0.03 に崩壊）・m2@44/m5@7 は世界依存（16seed で 0.31/0.62） |
...
| `tests/test_don_box.py` | **基盤健全性**（`cpu_infra`）。**ドン箱 DON_BOX**（`docs/cpu_don_box_plan.md` Phase 1・2026-08-12）: 配分計画のマクロ候補（付与k→リーダー攻撃）の列挙算術（k∈{通る最小,2枚要求}・予算上限・重複なし）／展開等価（箱適用＝原始列逐次適用）／実対局出力の原始手変換（`don_box_first_primitive`）／seam 既定OFF（`OPCGGame(don_box=)`）／action_key の k 分離。A/B 実測（(C)済み基準・200ペア）: **wr 0.518 CI[0.475,0.561]＝P1 単独は中立**（固有価値は P2=温存 r 待ち） |
