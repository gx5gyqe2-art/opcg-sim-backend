...
> 残課題（既知の限定対応）: 「キャラかドン合計N枚」の**混在選択**（1キャラ+1ドン）は未対応で
> Choice（キャラ最大N ‖ ドン最大N）で近似。「**キャラが自分の／相手の効果でレストになった時**」
> （任意主語＝PASSIVE 写像。OP07-031／OP10-036 等）の on-rest 誘発は未対応。
>
...
>
> **解消済み**: 「**このキャラがレストになった時**」（OP14-027/028/032/035/119 等）の on-rest 誘発は
> アタック宣言だけでなく**効果（REST アクション）によるレスト**でも発火する。`apply_action_to_engine`
> の REST 経路がアクティブ→レスト遷移で `_resolve_on_rest` を呼び、ターン文脈タグ（【自分/相手の
> ターン中】）で発火ターンを限定する（回帰 `tests/test_on_rest_trigger.py`）。
