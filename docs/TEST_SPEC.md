...
| `tests/scripts/mu_mcts_probe.py` | **make/unmake版MCTS（`tests/harness/az_mcts_tree_mu.py`）の実測プローブ**: ①clone版 `TreeMCTS` とのパリティ（確率非消費局面で同手・訪問数 max\|ΔN\|=0）②自己対戦の壁時計 ③`GameManager.clone` 呼び出し数を比較（`--sims`/`--positions`/`--bench-games`）。実測=deterministic局面パリティ 6/6・本走`_gen_task`(sims40)で ~2.7〜3.6×高速・clone ~98%減（探索中0・determinizeの1手1回のみ）。cProfile で自己対戦の79%がcloneだった件を `_apply_move_inplace`＋journal巻き戻しで消す。確率効果は run() の per-sim RNGリセットで**エッジごと固定（coherent）**＝clone と同一意味論（別の乱数実現）。本走への差し込みは `OPCG_MU_MCTS=1`（`p3_run.py`/`p3_loop.py` のドロップイン・既定OFF） |
