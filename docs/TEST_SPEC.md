...
| `tests/test_n_rel_train_torch.py` | **基盤健全性**（`cpu_infra`）。**NRel 訓練器の torch（CPU）学習経路**（2026-09-07・`rust_engine_plan.md` §18.4・`docs/reports/2026-09-07_train_torch.md`）: **torch が無ければ skip**（任意依存）。盤面は合成（乱数）＝dump も Python エンジンも要らない。a. 同じ重み・同じバッチで numpy 版と torch 版の value／policy logits が 1e-5 で一致／b. numpy の手書き backward と torch autograd の勾配が一致（|g|>1e-4 の要素で相対 <1e-4・max|Δg|/max|g| <1e-5・|g|>1e-6 では **float32 の丸めの下限**〔同じ numpy の式を加算順だけ変えて回した差〕の 5 倍以内。判定の式は `tests/harness/n_rel_torch_cmp.py`）／勾配が来るパラメータの集合が numpy と同じ（value は方策ヘッドを、policy は value ヘッドを触らない）／c. torch で訓練した npz を `n_rel.NRelNet.load`（serve の正本）が読めて forward 一致・鍵/形/dtype(float32)/`meta.kind=nrel-a`/`vocab_ids`/`ablate` の焼き込みが numpy 版のまま／d. `--backend` の選択と torch 非導入時の numpy への退避・numpy 参照実装が壊れていないこと・1 ステップ目の損失が numpy と一致 |
