...
| `tests/scripts/bb_gen.py` | **B系 bb1: 骨組みコーパス生成**（2026-08-12・`bb_train` の対）: バニラリーダー×合成カード（`bb_card_factory`）の自己対戦から1ターン1行（勝者 z ラベル・card_idx 省略＝ID情報なし）を採る。退化局は落とす。初回実測: 400局→8,652行 |
...
| `tests/scripts/bb_contested_isolation.py` | **B系 bb4b: 接戦帯の分離プローブ**（2026-08-13・`backbone_bb4_20260813.md` §4 の切り分け）: 実デッキ×**バニラリーダー**の対局を bb4 と同一の接戦フィルタ（turn≥4・ライフ差昇順・5点/局）＋同一教師正本（CR canon sims48×6世界 CRN・完走後ラベル）で採り、ID無しネットの r を**非飽和帯に層別**して測る。読み方: bb の非飽和 r が 0.4 級へ回復＝接戦盲目の主因はリーダー効果／0 のまま＝特徴不足（実カード能力意味論が物理特徴に映らない） |
