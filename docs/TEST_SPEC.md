...
| `tests/test_box_commit.py` | **基盤健全性**（`cpu_infra`）。**箱コミット実行の契約**（2026-08-26 ユーザ決定「箱は選ぶ時だけ判断し、中身は機械実行」）: アタック箱の完走（付与×k の後に必ず ATTACK＝半消化の再発ガード）／配分箱 k=2 のカウントダウン消化／契約違反（非合法 sig）で箱ごと破棄して通常判断へ縮退・クラッシュしない／box_commit=False seam は従来（毎 decide 判断）／PLAY 対話コミットが評価（window_pred=in_dialog・box_depth）と同一の選択になる |
