...
| `tests/scripts/power_threshold_probe.py` | **打点しきい値の未達スキャン**（v48・読み取り専用）: リプレイのフレームだけで「そのターン**しきい値以上の打点を作れたのに作らなかった**か」を数える。到達可能打点 = max(ターン開始時に場にいてレストでないキャラ／レストでないリーダーの素パワー + アクティブドン×1000)、実際 = そのターンの ATTACK の攻撃側パワー最大値。**用途**: 勝率が使えない対面の裁定裏取り——飽和負け局面ではレフェリー12世界でも ±1勝に揺れて識別できない（v48 実測: エネル turn 3 のプラン比較で人間の線と CPU の線が同価値バンド内）が、打点は盤面とドン枚数から決定論的に決まるのでノイズがゼロ。ユーザ裁定（2026-08-10）「エネルがナミに勝つには 7000 以上で殴る必要がある／キャラクターのエネル OP15-118 を出すのもパワー10000 が重要だから」を指標化したもの。**実測 2026-08-10（e1/e2・しきい値7000・gen13 自己対戦）**: 自ターン18件中 **5件が未達**。内訳は e1@5（到達11000／実際5000）・e2@2（7000／攻撃なし）・e2@6（11000／6000）・**e2@7 と e2@9（場に OP15-118 が立ち到達10000 なのに攻撃0＝活ドン0）**。e2@7/@9 は前ターンにドンを使い切る癖の裏返しで、同じエネルが turn 8/10 では 14000 まで伸びて殴っている＝隔ターンでしか機能していない。**射程の限定**: リーダー能力による一時的な打点増（エネルの「レストのドン4枚まで付与」等）は数えないので到達可能打点は**過小評価**＝偽陽性を出さない側に倒してあり、未達を全部拾えてはいない |
