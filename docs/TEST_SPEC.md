...
| `tests/scripts/plan_value_map.py` | **方針ヘッドの地図**（2026-09-12・§20.10）: 方針ヘッド付きネット（`n_rel_train --plan-weight`）を holdout の記録で読み、自席ターンの最初の main 行で V_face／V_board／V_mixed、相手ターンの最初の自分の行で V_take／V_guard を出す。argmax と実際に打った方針の一致率（agree）・V[argmax]−V[打った方針]（gap）・レースの状態（`race_state` の margin）× 相手手札 × ターン帯ごとの V_face−V_board と argmax の分布・一致／不一致の勝率・クラスごとの較正（mean V[played] vs mean z）・守りは自分のライフ別の V_guard−V_take。**注意**: 打った方針の列しか教師が無いので、その状態で滅多に打たれない方針の列は外挿＝地図は交絡込みで読む。読み取り専用 |
