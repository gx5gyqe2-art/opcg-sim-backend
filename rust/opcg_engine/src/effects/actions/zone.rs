//! 群 B（ゾーン移動: MOVE_CARD／DECK_BOTTOM／DECK_TOP／BOUNCE／TRASH_FROM_DECK／HEAL／DEAL_DAMAGE／SHUFFLE／ORDER_LIFE／FACE_UP_LIFE／LOOK_LIFE／MOVE_TO_HAND）。**WP `rs-p3-zone` が本体を入れる**（計画 `docs/rust_engine_plan.md` §11.7）。
//!
...
pub fn apply_target(
    ...
    Err(EngineError::Unimplemented(format!(
        "actions::zone: ActionType::{} は未実装（WP rs-p3-zone）",
        action.ty.name()
    )))
}
