//! 群 D（ドン!!: RETURN_DON／RAMP_DON／REST_DON／ATTACH_DON／ACTIVE_DON（target 無し）／FREEZE_DON／MOVE_ATTACHED_DON＋ドン!!を対象に取るクエリ）。**WP `rs-p3-don` が本体を入れる**（計画 `docs/rust_engine_plan.md` §11.7）。
//!
...
pub fn apply_target(
    ...
    Err(EngineError::Unimplemented(format!(
        "actions::don: ActionType::{} は未実装（WP rs-p3-don）",
        action.ty.name()
    )))
}
