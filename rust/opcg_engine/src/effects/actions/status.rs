//! 群 A（状態系: GRANT_KEYWORD／ATTACK_DISABLE／PREVENT_REST／FREEZE／NEGATE_EFFECT／DISABLE_ABILITY／SWAP_POWER・BUFF の全 status/期間）。**WP `rs-p3-status` が本体を入れる**（計画 `docs/rust_engine_plan.md` §11.7）。
//!
...
pub fn apply_target(
    ...
    Err(EngineError::Unimplemented(format!(
        "actions::status: ActionType::{} は未実装（WP rs-p3-status）",
        action.ty.name()
    )))
}
