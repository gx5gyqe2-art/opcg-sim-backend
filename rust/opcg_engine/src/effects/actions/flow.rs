//! 群 C（カードの流れ: PLAY_CARD／LOOK／REVEAL／SELECT／EXECUTE_MAIN_EFFECT／EXECUTE_EVENT／DECLARE_COST）。**WP `rs-p3-flow` が本体を入れる**（計画 `docs/rust_engine_plan.md` §11.7）。
//!
...
pub fn apply_target(
    ...
    Err(EngineError::Unimplemented(format!(
        "actions::flow: ActionType::{} は未実装（WP rs-p3-flow）",
        action.ty.name()
    )))
}
