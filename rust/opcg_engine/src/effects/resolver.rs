...
use super::{ability_of, abilities, EffectContext, NodeRef, NodeRoot};
...
impl Resolver {
    ...
    pub fn process_stack(
        ...
        while let Some(node_ref) = self.execution_stack.pop() {
            ...
            let node = node_ref.resolve(abilities()).cloned().ok_or_else(|| {
    ...
    pub fn resolve_targets(
        ...
        if query.select_mode == "REMAINING" {
            if let Some(group) = self.context.saved(SEL_GROUP_ID).cloned() {
                if !group.is_empty() {
        ...
        let mut candidates = super::get_target_cards(
            s.state(),
            masters,
            abilities(),
            query,
        ...
        if query.flags.iter().any(|f| f == "EXCLUDE_SELECTED_COLOR") {
            let ref_cards = self.context.saved("selected_card").cloned().unwrap_or_default();
    ...
    fn resolve_both_sides(
        ...
        for (side, side_name, side_player) in [
            ...
            let cand = super::get_target_cards(
                s.state(),
                masters,
                abilities(),
                &side_q,
        ...
        if let Some(save_id) = query.save_id.as_ref() {
            self.context.set_saved(save_id, result.clone());
        }
        Ok(Some(result))
    }
