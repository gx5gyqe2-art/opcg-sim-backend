...
fn every_suspension_kind_has_the_python_request_shape() {
    ...
    let mut masters_trigger = MasterTable {
        masters: masters.masters.clone(),
        by_id: masters.by_id.clone(),
    };
...
/// リーダーだけの盤面でも `board_with` が使えることを確認する足場テスト（回帰の土台）。
#[test]
fn the_test_table_is_shared_across_tests() {
    let a = testkit::effect_table() as *const _;
    let b = testkit::effect_table() as *const _;
    assert!(std::ptr::eq(a, b), "OnceLock なので同じ表を返す");
    assert!(!testkit::effect_table().abilities.is_empty());
    let _ = M_LEADER;
}
