...
`probe`（枝別出口の採点）と `gate`（coach 13 点）は **Python エンジンと旧 coach 計器を要る**ので
`legacy/python_engine/tests/scripts/n0_gate.py` へ分離した（2026-09-07・計画 §16.3-6: `opcg_sim/`
から `legacy/` を import する箇所を 0 にする）。本モジュールに残るのはカード表（`build_card_table`）・
ネット（`N0Net`）・訓練で、`n1_train` はここだけを使う。
"""
import os
...
def main():
    ...
    args = ap.parse_args()
    # `probe`／`gate` は `legacy/python_engine/tests/scripts/n0_gate.py` へ分離した（上の docstring）。
    train(args)
