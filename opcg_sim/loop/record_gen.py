...
**dump v4**（既定・2026-09-10・計画 §20.8）: v3 の列は 1 バイトも変えず、**補助教師の 3 列を
追加する**だけ（`aux`／`aux_tok`／`aux_mask`・WP `rs-aux-heads`）。同じ契約で
`deck_kinds` 列（WP `rs-removal-decks`）が並行して足される。`dump_io.py` は**無い列を
`None` で返す**＝v3 の波はそのまま読める。補助教師の中身は下の `aux_from_ledger` が正本。
"""
import os
...
DUMP_VERSION = 4       # v3 ＋ 補助教師の列（aux／aux_tok／aux_mask・§20.8）
...
def _init_worker(sims, net, dirichlet_eps, temp_turns, aux=True):
    E.engine()
...
def main(argv=None):
    ...
    keys = _ROW_KEYS + _POL_KEYS + _TOK_KEYS + (() if args.no_aux else _AUX_KEYS)
    buf = {k: [] for k in keys}
    shard = n_rows = n_drop = n_main = 0
    initargs = (args.sims, args.net, args.dirichlet_eps, args.temp_turns, not args.no_aux)
