"""make/unmake 版 ノード型 PUCT MCTS（clone廃止・自己対戦/本番の単一の正）。

`mcts.TreeMCTS`（clone版）はエッジごとに `GameManager.clone()`（deepcopy）で子状態を作り保持する。
cProfile 実測で自己対戦の 79% が clone。本実装は **状態をノードに持たせず**、1シミュレーションを
ルートの作業状態から降下しながら `_apply_move_inplace` で**その場適用**し、per-ply の
`journal.transaction()` 退出で**自動巻き戻し**する（製品α-βの `cpu_ai._recurse_child` と同一パターン＝
`test_cpu_make_unmake.py` で clone 同値を実証済み）。クローンは determinize の 1手1回だけ（探索中は0）。
訪問数 N/W はゲーム状態外の numpy 配列なので巻き戻し不変。
...
**配置（統一後）**: 本モジュールが唯一の正。`tests/harness/az_mcts_tree_mu.py` はここへの委譲shim。
本番 `cpu_learned` は既定 clone（`mcts.TreeMCTS`）・環境変数 `OPCG_MU_MCTS=1` で本 mu 版へ切替（clone版
とは「同一分布・別乱数実現」＝現行出荷挙動の変更になるためオプトイン。決定性/強度の検証を経て既定化）。
"""
import math
import random
...
class TreeMCTSMakeUnmake:
    ...
    def run(self, real_state):
        ...
        # トレース用の root 統計（clone版 TreeMCTS.last_stats と同形）。cpu_learned.decide の
        # 等価手マージ（_merge_root_stats）とトレース候補一覧（_fill_trace）はこれを読む＝
        # 無いと mu 版だけ等価手マージが効かず trace["candidates"] も欠落する（clone/mu の非対称）。
