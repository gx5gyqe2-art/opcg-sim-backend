...
# **既定 False（opt-in）**: 機構としては葉評価の既知の盲目を解消するが、単体では決定が
# 変わらないと実測（2026-08-04・8点で m1@15 は 0.00 のまま・m5@7 が 0.69→0.44＝ノイズ幅
# 0.35 未満だが低下方向）。**教師側の符号化も解決後へ揃えるまで**既定化しない
# （`defense_cf_gen` は戦闘途中の子を符号化しており、1000/2000 の子がほぼ同一入力に
# 異なるラベルを持つ＝学習不可能な教師になっていた。v34 で「教えても動かない」原因の疑い）。
SERVE_QUIESCE = False
