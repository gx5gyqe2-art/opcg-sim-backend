...
class Effect:
    ...
    def _parse(self):
        ...
        for part in parts:
            ...
            # 定数を使用して判定
            if _nfc(ParserKeyword.ON_PLAY) in part:
                trigger = TriggerType.ON_PLAY
            elif _nfc(ParserKeyword.ACTIVATE_MAIN) in part:
                trigger = TriggerType.ACTIVATE_MAIN
            elif _nfc(ParserKeyword.WHEN_ATTACKING) in part:
                trigger = TriggerType.WHEN_ATTACKING
            elif _nfc(ParserKeyword.ON_KO) in part:
                trigger = TriggerType.ON_KO
            elif _nfc(ParserKeyword.MY_TURN) in part:
                trigger = TriggerType.CONST_EFFECT
            elif _nfc(ParserKeyword.OPPONENT_TURN) in part:
                trigger = TriggerType.CONST_EFFECT
    ...
    def _parse_single_action(self, text: str, subject: Player, is_cost: bool) -> List[EffectAction]:
        ...
        # 定数を使用してアクションタイプを判定
        if _nfc(ParserKeyword.DRAW) in text:
            act_type = ActionType.DRAW
        ...
        # コスト支払いの特殊判定 (is_cost=True の場合)
        if is_cost:
            if _nfc(ParserKeyword.DON) in text:
                act_type = ActionType.DON_COST
            elif _nfc(ParserKeyword.HAND) in text:
                act_type = ActionType.MOVE_CARD
                dest_zone = Zone.TRASH
            elif _nfc(ParserKeyword.TRASH) in text:
                act_type = ActionType.MOVE_CARD
    ...
    def _parse_condition(self, text: str) -> Optional[Condition]:
        ...
        if _nfc(ParserKeyword.TRASH) in text:
            ...
            if nums:
                val = int(nums[0])
                op = CompareOperator.GE if _nfc("以上") in text else CompareOperator.LE
        ...
        if _nfc(ParserKeyword.LIFE) in text:
            ...
            if nums:
                val = int(nums[0])
                op = CompareOperator.LE if _nfc("以下") in text else CompareOperator.GE
                return Condition(ConditionType.LIFE_COUNT, operator=op, value=val, raw_text=text)

        if _nfc(ParserKeyword.LEADER) in text and _nfc("が") in text:
