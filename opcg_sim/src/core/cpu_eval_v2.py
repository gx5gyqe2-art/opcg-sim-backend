...
V2_W_LIFE_PRECIOUS = 2.21  # 薄域（先頭 knee 枚）のライフ 1 枚＝守るべき・支配項（SPSA x1.10）
V2_W_LIFE_HIGH     = 0.80  # 厚域（knee 超）のライフ 1 枚＝安い（受ける）
V2_LIFE_KNEE       = 2     # 凹型の膝（lethal 警戒枚数）
V2_W_DECK   = 0.50     # デッキ切れ側の敗北距離（ライフと同じ通貨・危険域1枚あたり）

V2_W_DEV    = 0.77     # 展開可能なカード1枚の汎用価値（効果別にしない＝§4.1。SPSA x1.28）
V2_W_CTR    = 0.33     # カウンター1000あたりのカード換算（SPSA x0.83）
V2_KAPPA    = 1.0      # 展開割引 γ_surv の指数（生存ターン感応の強さ）
V2_LAMBDA   = 0.98     # カウンター増幅 amp の係数
V2_OPP_HAND_UPLIFT = 1.2  # 相手手札（枚数ベース）の上振れ補正（自分は貪欲最適＝楽観バイアス補正・§Q4b）

V2_W_BODY   = 0.36     # 盤面の体＝トレード/制圧価値（パワーでスケール）。顔打点は持たない（SPSA x0.91）
V2_REST_DISCOUNT = 0.5    # レスト体（露出リスク）の割引（自ターン終了が葉のときのみ・§Q3）

V2_W_TELE   = 0.5      # Tele（ホライズン端のリーサル番兵）の重み。主役は凹型ライフ＝控えめ

V2_W_DON    = 0.18     # アクティブドンの床（ランプ＋構え防御オプション・小・SPSA x0.91）

# 上記は SPSA 第1パス（8iter/6games）の best を **30局・未学習シードで検証**して確定した値。
# 検証: v2 ON vs 評価OFF(成熟J値) ＝ 14/30＝46.7%・Elo -23（CI[-145,+99]＝互角）。校正直後の 5/30(Elo-280)
# から +257Elo。汎化（SPSA 学習シードと別シード）。さらに局数を増やした SPSA で詰める余地あり（§9）。
