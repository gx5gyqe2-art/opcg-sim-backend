...
# ② 自明手の即答（Phase 3・OPCG_TRIVIAL_FASTPATH=1・既定 OFF）: 1-ply で best が他の全ルート手を
# _TRIVIAL_MARGIN 以上に圧倒し、かつ best が重要手（B-3）でない局面は、深掘りを省いて即答する
# （明らかに最善＝深く読んでも覆りにくい）。1-ply は自ターン途中の楽観値で稀に深掘りが覆すため挙動変化
# ＝既定 OFF（テスト/自己対戦は従来同値）・本番は A/B で強さ非退行を確認のうえ有効化する。
_TRIVIAL_FASTPATH = os.environ.get("OPCG_TRIVIAL_FASTPATH", "0") == "1"
_TRIVIAL_MARGIN = 12000.0  # 1-ply の best−2nd がこの差以上で自明扱い（≈1.5 ライフ・保守的）
...
def _scored_search(manager, name: str, moves: List[Dict[str, Any]],
    ...
    # ② 自明手の即答（OPCG_TRIVIAL_FASTPATH・既定 OFF）: 1-ply で best が 2nd を _TRIVIAL_MARGIN 以上に
    # 圧倒し、best が重要手(B-3)でも TURN_END でもない局面は、深掘りを省いて即答する（明らかに最善＝深く
    # 読んでも覆りにくい）。best のみ返すと decide が max で best を選ぶ（fold は TURN_END 不在で非作動）。
    if _TRIVIAL_FASTPATH and len(order) >= 2:
        bi = order[0]
        b_s, b_m, b_ok = prelim[bi]
        if (b_ok and not imp_by_idx.get(bi, False)
                and b_m.get("action_type") != "TURN_END"
                and b_s - prelim[order[1]][0] >= _TRIVIAL_MARGIN):
            if collect is not None:
                collect.setdefault("prelim", {})
                collect.setdefault("deep", {})
                for s1, mm, ok in prelim:
                    collect["prelim"][_move_sig(mm)] = s1
                collect["deep"][_move_sig(b_m)] = b_s
