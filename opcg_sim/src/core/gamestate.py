...
class GameManager:
    ...
    def _apply_action_target_loop(self, player: Player, action: GameAction, targets: List[CardInstance], value: int, source_card: Optional[CardInstance] = None) -> bool:
        # プレイヤーレベル・ハンドラに該当しなかったアクションの対象ループ（除去保護/置換ゲート・
        # success 規約を含む）。旧 apply_action_to_engine 後半を逐語移設。
