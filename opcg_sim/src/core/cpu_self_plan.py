...
# マッチアップ補正（Phase 2・§2.5.5）: 相手リーダー推測 `OpponentProfile`（§2.5.4）で理想ラインの傾きを補正。
_MATCHUP_AGGRO_K = 0.5         # 速い相手（aggro_lean 高）ほど前倒し＝傾きを急に（レース前に差を作る）
_MATCHUP_DEF_K = 0.6          # ブロッカー/除去の厚い相手ほど後ろ倒し＝傾きを緩く（トレードで遅れる前提）
_MATCHUP_MULT_MIN = 0.6
_MATCHUP_MULT_MAX = 1.5


def _matchup_slope_mult(opp_profile) -> float:
    """相手プロファイルから理想ラインの傾き乗数を出す（None＝1.0＝マッチアップ補正なし）。

    速い相手は前倒し（>1）／受け・除去の厚い相手は後ろ倒し（<1）。参照は相手リーダー紐付けの
    テンプレ由来集計のみ（実手札・実デッキは読まない＝フェア・§2.5.4）。
    """
    if opp_profile is None:
        return 1.0
    fast = _MATCHUP_AGGRO_K * (getattr(opp_profile, "aggro_lean", 0.5) - 0.5)
    grind = _MATCHUP_DEF_K * ((getattr(opp_profile, "blocker_ratio", 0.0)
                               + getattr(opp_profile, "removal_ratio", 0.0)) / 2.0)
    return max(_MATCHUP_MULT_MIN, min(_MATCHUP_MULT_MAX, 1.0 + fast - grind))


def _derive_delta_schedule(aggro_lean: float, removal_ratio: float, opp_profile=None) -> tuple:
    """構成（＋相手プロファイル）から理想 J値差スケジュールを線形近似で導出。

    自デッキ: 攻め寄り・除去多いほど傾きが急＝早ターンで差を開く理想。マッチアップ補正（Phase 2）:
    相手が速いほど前倒し／受け厚いほど後ろ倒し。`cpu_ai._plan_progress` が実測 J値差との乖離
    （スケジュール遵守度）を採点する。フェア性: 自デッキ構成＋相手テンプレ集計のみ。
    """
    slope = _SCHED_SLOPE_BASE + _SCHED_SLOPE_AGGRO * aggro_lean + _SCHED_SLOPE_REMOVAL * removal_ratio
    slope *= _matchup_slope_mult(opp_profile)
    slope = max(_SCHED_SLOPE_BASE, min(_SCHED_SLOPE_MAX, slope))
    return tuple(round(slope * t, 3) for t in range(_SCHED_TURNS + 1))


def build_plan(masters: List, leader: Optional[object] = None, opp_profile=None) -> PlanProfile:
    """自デッキのカード定義（CardMaster の列・リーダー/ドンは除外して渡す想定）からプランを作る。

    空入力・分類不能でも安全に NEUTRAL（≒現行挙動）を返す。`leader` は将来のリーダー別補正用に
    受け取るが現状は未使用。`opp_profile`（相手リーダー推測の `OpponentProfile`・§2.5.4。normal で供給・
    None 可）があれば**理想ライン（J値スケジュール）の傾きをマッチアップ補正**する（Phase 2・§2.5.5）。
    """
    ...
    return PlanProfile(
        ...
        delta_schedule=_derive_delta_schedule(prof.aggro_lean, prof.removal_ratio, opp_profile),
        **p,
    )
