...
class SandboxManager:
    ...
    def _refresh_player(self, pid: str):
        p = self.state[pid]
        if p["leader"]: p["leader"].is_rest = False
        if p["stage"]: p["stage"].is_rest = False
        for c in p["field"]: c.is_rest = False
        
        for d in p["don_rested"]:
            d.is_rest = False
            p["don_active"].append(d)
        p["don_rested"] = []
        
        for d in p["don_attached"]:
            d.is_rest = False
            d.attached_to = None
            p["don_active"].append(d)
        p["don_attached"] = []
    ...
    def start_turn_process(self):
        ...
        p = self.state[pid]
        self._refresh_player(pid)
        
        if p["deck"]:
            p["hand"].append(p["deck"].pop(0))
        
        don_add_count = 2
        if self.turn_count == 1: don_add_count = 1
        
        for _ in range(don_add_count):
            if p["don_deck"]:
                p["don_active"].append(p["don_deck"].pop(0))
                
        log_event("INFO", "sandbox.turn_start", f"Turn {self.turn_count} started for {pid}", player="system")

    def end_turn_process(self):
        self.active_player_id = "p2" if self.active_player_id == "p1" else "p1"
        self.turn_count += 1
        log_event("INFO", "sandbox.turn_end", f"Turn switched to {self.active_player_id}", player="system")
