...
配線は tests/ の学習コードを legacy/python_engine/learned へ**忠実コピー**した単一ソース。コピーが訓練時の
...
from legacy.python_engine.core import cpu_learned
from opcg_sim.learned import encoder as PROD_E
from legacy.python_engine.learned.action import legal_action_matrix as prod_lam
...
def test_learned_enumerates_optional_confirm():
    ...
    from legacy.python_engine.learned.adapter import OPCGGame
    gm, p1, _ = make_game()
    for i in range(3):
...
def test_selection_merge_key_distinguishes_position():
    ...
    from legacy.python_engine.core.cpu_ai import _selection_merge_key
...
def test_encoder_no_drift():
    ...
    注（重複解消後）: `tests/harness/rl_encoder.py` は本番 `opcg_sim.learned.encoder` への
...
def _gen2_vnet():
    ...
    import os
    from legacy.python_engine.learned.value_net import ValueNet
...
def test_warm_start_policy_is_identity():
    ...
    import os
    from legacy.python_engine.learned.policy import PolicyScorer, state_context
    from legacy.python_engine.learned.action import legal_action_matrix
...
def test_action_features_no_drift():
    """注（重複解消後）: `opcg_action.py` は本番 `legacy.python_engine.learned.action` への委譲shim＝
