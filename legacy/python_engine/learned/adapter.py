...
class OPCGGame:
    ...
    def legal_actions(self, state):
        ...
        if pf is None:   # インスタンス未指定＝config（serve 既定）に従う
            from opcg_sim.learned.config import SERVE_PRUNE_FUTILE
        ...
        mm = self.macro_moves
        if mm is None:
            from opcg_sim.learned.config import SERVE_MACRO_MOVES
        ...
        if dbx is None:   # インスタンス未指定＝config（serve 既定）に従う
            from opcg_sim.learned.config import SERVE_DEFENSE_BOX
