...
**本ファイルは CPU 評価の単一系統（L1）**。`cpu_ai.evaluate_base` はこの `evaluate_v2` をそのまま返す。
数式の構造は v0.4 に忠実だが、**係数（カーブの形・スケール）は未チューニング**＝この後アリーナA/B＋SPSA で
詰める（§9）。stdlib-only・PyPy互換・読み取りのみ決定論。
...
# 全体スケール（カード単位 → 探索閾値と同オーダーへ）。探索側の閾値（_ACT_MARGIN=300・ビーム剪定・
# settle 判定等）は ±数千スケール前提でハードコードされている。
# L1 をカード単位（±10）のまま返すと _ACT_MARGIN が相対的に巨大化し「常に何もしない」になる。
