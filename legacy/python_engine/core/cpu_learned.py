...
from opcg_sim.learned import config as CFG
from opcg_sim.learned.config import (
    C_PUCT, SERVE_SIMS, SERVE_DIRICHLET_EPS, SERVE_STICKY_WORLD)
from legacy.python_engine.learned.mcts import (   # make/unmake版（唯一の探索実装。旧clone版は削除済み）
...
# c10 は効果構造符号化ネット（`opcg_sim/learned/n_eff.py`・カードIDの埋め込みを持たず
...
# a1 は N系 v3（NRel Stage A・`opcg_sim/learned/n_rel.py`）: 盤面を 22 枠の「効果構造＋今の状態
...
class LearnedEngine:
    ...
    def __init__(self, value_path: Optional[str] = None, policy_path: Optional[str] = None,
        ...
        if (don_margin is not None or macro_moves is not None
            ...
            from legacy.python_engine.learned.adapter import OPCGGame as _OG
        ...
        if _is_nrel(vp):
            ...
            from opcg_sim.learned import n_rel as _NL
        ...
        if self.pnet is not None:
            from legacy.python_engine.learned.action import ACTION_DIM
